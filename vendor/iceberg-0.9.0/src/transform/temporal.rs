    ...
    fn transform_literal(&self, input: &crate::spec::Datum) -> Result<Option<crate::spec::Datum>> {
        let val = match (input.data_type(), input.literal()) {
            (PrimitiveType::Date, PrimitiveLiteral::Int(v)) => {
                Date32Type::to_naive_date_opt(*v).expect("Date32Type::to_naive_date_opt overflow").year() - UNIX_EPOCH_YEAR
            }
...
impl TransformFunction for Month {
    ...
    fn transform_literal(&self, input: &crate::spec::Datum) -> Result<Option<crate::spec::Datum>> {
        let val = match (input.data_type(), input.literal()) {
            (PrimitiveType::Date, PrimitiveLiteral::Int(v)) => {
                (Date32Type::to_naive_date_opt(*v).expect("Date32Type::to_naive_date_opt overflow").year() - UNIX_EPOCH_YEAR) * 12
                    + Date32Type::to_naive_date_opt(*v).expect("Date32Type::to_naive_date_opt overflow").month0() as i32
            }
            (PrimitiveType::Timestamp, PrimitiveLiteral::Long(v)) => {
