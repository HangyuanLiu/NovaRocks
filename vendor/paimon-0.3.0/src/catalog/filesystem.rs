...
impl FileSystemCatalog {
    ...
    pub fn new(options: Options) -> crate::Result<Self> {
        let warehouse = required_warehouse(&options)?;
    ...
    /// Create a filesystem catalog using an already-authorized FileIO.
    ///
    /// The caller owns storage credentials and request resources. This path
    /// does not inspect storage properties or construct a storage client.
    pub fn with_file_io(options: Options, file_io: FileIO) -> crate::Result<Self> {
        let warehouse = required_warehouse(&options)?;
        Ok(Self { file_io, warehouse })
    }
    ...
    async fn list_directories_plain(&self, path: &str) -> Result<Vec<String>> {
        ...
        let dirs = statuses
            ...
            .filter_map(|status| {
                if status.is_dir {
                    get_basename(status.path.as_str())
                        // opendal get_basename contains "/" for a directory.
                        .strip_suffix("/")
                        .map(str::to_string)
                } else {
                    None
...
fn required_warehouse(options: &Options) -> crate::Result<String> {
    options
