    ...
    R: AsyncRead + Send + Unpin,
{
    ...
    pub async fn init_after_ssl(
        ...
        let scramble = self.shim.salt();
        {
            ...
            // Only let the response writer observe capabilities negotiated by
            // both peers. In particular, a client cannot opt into multiple
            // results unless the server advertised that protocol surface.
            self.client_capabilities = handshake.capabilities & self.client_capabilities;
