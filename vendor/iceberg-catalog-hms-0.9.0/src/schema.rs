...
impl SchemaVisitor for HiveSchemaBuilder {
    ...
    fn primitive(&mut self, p: &iceberg::spec::PrimitiveType) -> iceberg::Result<String> {
        let hive_type = match p {
            ...
            // NovaRocks vendor PATCH: the vendored iceberg adds
            // `PrimitiveType::Variant` (Iceberg v3 unshredded variant), which
            // crates.io iceberg-catalog-hms 0.9.0 does not know about. Hive has
            // no native variant type, so reject it rather than guessing — same
            // fail-fast treatment as the timestamptz variants.
