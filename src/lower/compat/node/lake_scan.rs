...
use crate::lower::compat::node::decode::build_scan_query_global_dicts;
use crate::lower::compat::node::{Lowered, QueryGlobalDictMap, local_rf_waiting_set};
...
pub(crate) fn lower_lake_scan_node(
    ...
    for entry @ (_tup_id, slot_id_raw) in &scan_layout.order {
        let found_virtual = 'check: {
            ...
            let Some(arrow_type) =
                crate::lower::compat::type_lowering::arrow_type_from_desc(slot_type)
            else {
                break 'check false;
            };
