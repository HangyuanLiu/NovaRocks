...
mod tests {
    ...
    #[cfg(feature = "compat")]
    #[test]
    fn lowers_starrocks_native_scan_to_existing_scan_config() {
        let mut node = scan_node_with(
            vec![
                output_column(1, "id", DataType::Int64),
                output_column(2, "flag", DataType::Boolean),
            ],
            vec![equals(
                column_ref(2, "flag", DataType::Boolean),
                expr::Expr {
                    r#type: Some(type_desc(&DataType::Boolean)),
                    nullable: false,
                    kind: Some(expr::expr::Kind::Literal(expr::LiteralExpr {
                        value: Some(common::LiteralValue {
                            value: Some(common::literal_value::Value::BoolValue(false)),
                        }),
                    })),
                },
            )],
        ...
        node.limit = 25;
        let _registry_guard = crate::runtime::starlet_shard_registry::lock_for_test();
        crate::runtime::starlet_shard_registry::clear_for_test();
        assert_eq!(
            crate::runtime::starlet_shard_registry::upsert_many([(
                300,
                "/warehouse/db/t/partition_100/tablet_300".to_string(),
            )]),
            1
        );
        ...
        let ctx = NodeLoweringContext::default()
            ...
            .with_query_id(crate::common::types::UniqueId { hi: 41, lo: 42 })
            .with_query_options(Some(crate::runtime::query_options::QueryOptions {
                batch_size: Some(512),
                query_timeout: Some(9),
                exec_mem_limit: Some(1_024),
                ..Default::default()
            }))
        ...
        let lowered = crate::lower::novarocks::lower_proto_node(&node, &mut arena, &ctx)
            .expect("lower native StarRocks scan");
        let ExecNodeKind::Limit(limit) = lowered.node.kind else {
            panic!("expected distributed Limit");
        };
        let ExecNodeKind::Scan(scan) = limit.input.kind else {
            panic!("expected Scan below distributed Limit");
        };
        ...
        let cfg = captured
            ...
            .clone()
            .expect("captured StarRocks config");
        assert_eq!(cfg.db_name.as_deref(), Some("db"));
        assert_eq!(cfg.table_name.as_deref(), Some("t"));
        assert_eq!(cfg.ranges.len(), 1);
        assert_eq!(cfg.ranges[0].tablet_id, 300);
        assert_eq!(cfg.ranges[0].partition_id, Some(100));
        assert_eq!(cfg.ranges[0].version, Some(7));
        assert_eq!(cfg.limit, Some(25));
        assert_eq!(cfg.batch_size, Some(512));
        assert_eq!(cfg.query_timeout, Some(9));
        assert_eq!(cfg.mem_limit, Some(1_024));
        assert_eq!(
            cfg.required_chunk_schema.slot_ids(),
            &[SlotId::new(1), SlotId::new(2)]
        );
        assert_eq!(cfg.required_chunk_schema.slots()[0].unique_id(), Some(0));
        assert_eq!(cfg.required_chunk_schema.slots()[1].unique_id(), Some(12));
        assert_eq!(
            cfg.output_chunk_schema.slot_ids(),
            &[SlotId::new(1), SlotId::new(2)]
        );
        let partition_paths: std::collections::BTreeMap<String, String> = serde_json::from_str(
            cfg.properties
                .get("partition_storage_paths")
                .expect("partition storage paths property"),
        )
        .expect("partition storage paths JSON");
        assert_eq!(
            partition_paths.get("100").map(String::as_str),
            Some("/warehouse/db/t/partition_100")
        );
        let meta = cfg.lake_schema_meta.expect("StarRocks schema identity");
        assert_eq!((meta.db_id, meta.table_id, meta.schema_id), (10, 20, 30));
        let native_schema = meta
            .native_tablet_schema
            .as_ref()
            .expect("authoritative native tablet schema");
        assert_eq!(native_schema.id, Some(30));
        assert_eq!(
            native_schema.keys_type,
            Some(crate::service::grpc_client::proto::starrocks::KeysType::DupKeys as i32)
        );
        assert_eq!(native_schema.sort_key_idxes, vec![0]);
        assert_eq!(native_schema.sort_key_unique_ids, vec![0]);
        assert_eq!(native_schema.column[0].r#type, "BIGINT");
        assert_eq!(native_schema.column[0].is_key, Some(true));
        assert_eq!(
            native_schema.column[1].default_value.as_deref(),
            Some(b"false".as_slice())
        );
        let native_hints = meta
            .native_column_hints
            .as_ref()
            .expect("wire-free native schema hints");
        assert_eq!(native_hints.len(), 2);
        assert_eq!(
            (
                native_hints[0].name.as_str(),
                native_hints[0].unique_id,
                native_hints[0].default_value.as_deref(),
            ),
            ("id", 0, None)
        );
        assert_eq!(
            (
                native_hints[1].name.as_str(),
                native_hints[1].unique_id,
                native_hints[1].default_value.as_deref(),
            ),
            ("flag", 12, Some("false"))
        );
        assert_eq!(meta.query_id.map(|id| (id.hi, id.lo)), Some((41, 42)));
    }

    #[cfg(feature = "compat")]
    #[test]
    fn starrocks_scan_keeps_predicate_columns_below_final_project() {
        let scan = scan_node_with(
            vec![
                output_column(1, "id", DataType::Int64),
                output_column(2, "flag", DataType::Boolean),
            ],
        ...
        let project = plan::DistributedNode {
            node_id: 20,
            fragment_id: 0,
            tuple_ids: Vec::new(),
            nullable_tuple_ids: Vec::new(),
            ...
            payload: Some(plan::distributed_node::Payload::Physical(plan::PlanNode {
                ...
                kind: Some(plan::plan_node::Kind::Project(plan::ProjectNode {
                    items: vec![plan::ProjectItem {
                        expr: Some(column_ref(1, "id", DataType::Int64)),
        ...
        crate::runtime::starlet_shard_registry::upsert_many([(
            301,
            "/warehouse/db/t/partition_100/tablet_301".to_string(),
        )]);
        let (connectors, captured) = capturing_starrocks_registry();
        let ctx = NodeLoweringContext::default()
            .with_connector_registry(connectors)
            .with_scan_ranges(10, vec![starrocks_range(301, 100, 7)]);
        let mut arena = ExprArena::default();

        let lowered = crate::lower::novarocks::lower_proto_node(&project, &mut arena, &ctx)
            .expect("lower projected StarRocks scan");
        assert!(matches!(lowered.node.kind, ExecNodeKind::Project(_)));
    ...
    #[cfg(feature = "compat")]
    #[test]
    fn rejects_starrocks_native_scan_range_and_source_mismatches() {
        let node = scan_node(starrocks_source());
        let (connectors, _) = capturing_starrocks_registry();
        let ctx = NodeLoweringContext::default()
            .with_connector_registry(connectors)
            .with_scan_ranges(10, vec![file_range()]);
        let mut arena = ExprArena::default();
        let err = crate::lower::novarocks::lower_proto_node(&node, &mut arena, &ctx)
            .expect_err("StarRocks source with file range must fail");
        assert!(err.contains("expected StarRocks tablet range"), "{err}");

        let mut node = scan_node(starrocks_source());
        ...
        let Some(plan::scan_source::Kind::StarrocksTable(source)) = scan
            .table
            .as_mut()
            .and_then(|table| table.source.as_mut())
            .and_then(|source| source.kind.as_mut())
        else {
            panic!("expected StarRocks source");
        };
        source.schema_id = 0;
        let (connectors, _) = capturing_starrocks_registry();
        let ctx = NodeLoweringContext::default()
            ...
            .with_scan_ranges(10, vec![starrocks_range(300, 100, 7)]);
        let err = crate::lower::novarocks::lower_proto_node(&node, &mut arena, &ctx)
            .expect_err("non-positive schema id must fail");
        assert!(err.contains("schema_id must be positive"), "{err}");
    }

    #[cfg(feature = "compat")]
    #[test]
    fn rejects_incomplete_or_ambiguous_starrocks_current_schema() {
        fn source_mut(node: &mut plan::DistributedNode) -> &mut plan::StarRocksTableSource {
            let Some(plan::distributed_node::Payload::Physical(physical)) = node.payload.as_mut()
            else {
                panic!("expected physical node");
            };
        ...
        let mut missing = scan_node(starrocks_source());
        source_mut(&mut missing).current_schema = None;
        let err = crate::lower::novarocks::lower_proto_node(&missing, &mut arena, &ctx)
            .expect_err("missing full current schema must fail");
        assert!(err.contains("current_schema must be present"), "{err}");

        let mut unknown_sort_uid = scan_node(starrocks_source());
        source_mut(&mut unknown_sort_uid)
            .current_schema
            .as_mut()
            .expect("current schema")
            .sort_key_unique_ids = vec![999];
        let err = crate::lower::novarocks::lower_proto_node(&unknown_sort_uid, &mut arena, &ctx)
            .expect_err("unknown sort-key unique id must fail");
        assert!(err.contains("unknown unique_id 999"), "{err}");

        let mut duplicate_struct_child = scan_node(starrocks_source());
        let column = &mut source_mut(&mut duplicate_struct_child)
            .current_schema
            .as_mut()
            .expect("current schema")
            .columns[1];
        column.physical_type = "STRUCT".to_string();
        let child = plan::StarRocksColumnSchema {
            unique_id: 21,
        ...
        column.children = vec![child.clone(), child];
        let err =
            crate::lower::novarocks::lower_proto_node(&duplicate_struct_child, &mut arena, &ctx)
                .expect_err("duplicate STRUCT child identity must fail");
        assert!(err.contains("duplicate child name"), "{err}");
    }

    #[cfg(feature = "compat")]
    #[test]
    fn rejects_starrocks_native_scan_duplicate_tablets_and_storage_ids() {
        let node = scan_node(starrocks_source());
        let (connectors, _) = capturing_starrocks_registry();
        let ctx = NodeLoweringContext::default()
            .with_connector_registry(connectors)
            .with_scan_ranges(
                10,
                vec![starrocks_range(300, 100, 7), starrocks_range(300, 101, 8)],
            );
        let mut arena = ExprArena::default();
        let err = crate::lower::novarocks::lower_proto_node(&node, &mut arena, &ctx)
            .expect_err("duplicate tablet must fail");
        assert!(err.contains("duplicate tablet_id=300"), "{err}");
        ...
        source.storage_columns[1].unique_id = 0;
        let (connectors, _) = capturing_starrocks_registry();
        let ctx = NodeLoweringContext::default()
            .with_connector_registry(connectors)
            .with_scan_ranges(10, vec![starrocks_range(300, 100, 7)]);
        let err = crate::lower::novarocks::lower_proto_node(&node, &mut arena, &ctx)
            .expect_err("duplicate storage id must fail");
        assert!(err.contains("duplicate unique_id 0"), "{err}");
    }

    #[cfg(feature = "compat")]
    #[test]
    fn starrocks_empty_markers_never_create_tablet_scans() {
        let node = scan_node(starrocks_source());
        let (connectors, captured) = capturing_starrocks_registry();
        let carried_range =
            novarocks::scan_range::Kind::StarrocksTablet(novarocks::StarRocksTabletScanRange {
                tablet_id: 300,
                partition_id: 100,
                version: 7,
            });
        let ctx = NodeLoweringContext::default()
            ...
            .with_scan_ranges(
                10,
                vec![
                    starrocks_empty_range(None),
                    starrocks_empty_range(Some(carried_range)),
                ],
            );
        let mut arena = ExprArena::default();

        crate::lower::novarocks::lower_proto_node(&node, &mut arena, &ctx)
            .expect("empty StarRocks range markers must lower without tablet access");
        let cfg = captured
            .lock()
            .expect("captured StarRocks config lock")
            .clone()
    ...
    #[cfg(feature = "compat")]
    #[test]
    fn starrocks_native_ranges_reject_has_more() {
        let node = scan_node(starrocks_source());
        let _registry_guard = crate::runtime::starlet_shard_registry::lock_for_test();
        crate::runtime::starlet_shard_registry::clear_for_test();
        crate::runtime::starlet_shard_registry::upsert_many([(
        ...
        let (connectors, _) = capturing_starrocks_registry();
        let mut range = starrocks_range(300, 100, 7);
        range.has_more = Some(true);
        let ctx = NodeLoweringContext::default()
            .with_connector_registry(connectors)
        ...
        let err = crate::lower::novarocks::lower_proto_node(&node, &mut arena, &ctx)
            .expect_err("native StarRocks range continuation must fail");
        assert!(err.contains("has_more=true"), "{err}");
    }

    #[cfg(feature = "compat")]
    #[test]
    fn starrocks_native_scan_extracts_supported_min_max_and_keeps_full_residual() {
        let node = scan_node_with(
            ...
            vec![
                greater_than(column_ref(1, "id", DataType::Int64), int_literal(10)),
                not_equals(
                    column_ref(2, "flag", DataType::Boolean),
                    expr::Expr {
                        r#type: Some(type_desc(&DataType::Boolean)),
        ...
        let lowered = crate::lower::novarocks::lower_proto_node(&node, &mut arena, &ctx)
            .expect("lower StarRocks scan with pruning predicate");
        let ExecNodeKind::Scan(scan) = lowered.node.kind else {
            panic!("expected StarRocks scan node");
        };
        let residual = scan
            .conjunct_predicate()
            .expect("full residual conjunct must remain");
        assert!(matches!(arena.node(residual), Some(ExprNode::And(_, _))));
        let cfg = captured
            .lock()
            .expect("captured StarRocks config lock")
            .clone()
