        ...
        match sink.type_ {
            ...
            data_sinks::TDataSinkType::SPLIT_DATA_STREAM_SINK => {
                ...
                for expr in split_exprs {
                    split_expr_ids.push(crate::lower::compact::expr::lower_t_expr(
                        expr,
                        &mut exec_plan.arena,
                        &lowered.layout,
                        last_query_id,
...
mod tests {
    ...
    fn iceberg_dv_sink_lowers_to_deletion_vectors_mode() {
        let mode = crate::lower::compact::sink::iceberg::iceberg_sink_mode_for_type(
            data_sinks::TDataSinkType::ICEBERG_DV_SINK,
        );
        ...
        assert_eq!(
            crate::lower::compact::sink::iceberg::iceberg_sink_mode_for_type(
                data_sinks::TDataSinkType::ICEBERG_DELETE_SINK
            ),
    ...
    fn iceberg_equality_delete_sink_lowers_to_equality_deletes_mode() {
        ...
        assert_eq!(
            crate::lower::compact::sink::iceberg::iceberg_sink_mode_for_type(sink_type),
