//! StarRocks table CREATE TABLE AS SELECT (CTAS).
//!
//! Mirrors `iceberg_ctas` but targets the StarRocks table backend. The flow:
//! 1. Resolve the target table name to (namespace, table).
//! 2. Run the SELECT once to recover the result-set schema.
//! 3. Convert that schema into `TableColumnDef`s and create the StarRocks table
//!    via the StarRocks DDL path.
//! 4. Re-run the SELECT as an INSERT INTO ... SELECT so the rows land in
//!    the newly-created table through the standard StarRocks sink.
//!
...
pub(crate) fn execute_starrocks_table_ctas(
    ...
    debug_assert!(
        ...
        "StarRocks table CTAS dispatch requires AS SELECT"
    );
    ...
    // IF NOT EXISTS / already-exists check against the StarRocks table catalog.
    let already_exists = {
        let lake = state
            .starrocks_table
            .read()
    ...
    if already_exists {
        ...
        return Err(format!(
            "StarRocks table CTAS failed: table {}.{} already exists",
    ...
    // StarRocks sink owns the write path).
    let query = stmt
        .as_select
        .as_ref()
        .expect("StarRocks table CTAS requires AS SELECT")
        .clone();
    let (_, schema_cols) =
        crate::engine::iceberg_writer::run_select_to_chunks_and_schema(state, &target, &query)?;
    if schema_cols.is_empty() {
        return Err(
            "StarRocks table CTAS: SELECT produced no output columns; schema cannot be inferred"
                .to_string(),
        );
    }
    ...
    // Step B: create the StarRocks table with the inferred schema.
    let table_object = crate::sql::parser::ast::ObjectName {
        parts: vec![target.table.clone()],
    };
    crate::connector::starrocks::table::ddl::create_starrocks_table(
        state.as_ref(),
        &table_object,
        &target.namespace,
        &columns,
    ...
    if let Err(err) = insert_result {
        ...
        let _ = crate::connector::starrocks::table::ddl::drop_starrocks_table(
            state,
            &target.namespace,
            &target.table,
        );
        return Err(format!(
            "StarRocks table CTAS failed during data write: {err}"
        ));
    }
...
/// the StarRocks table `create_table` API. Reuses the iceberg helper for the
