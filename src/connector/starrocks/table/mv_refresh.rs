...
mod tests {
    ...
    use crate::connector::starrocks::table::model::{
        StarRocksGlobalMeta, StarRocksIndexState, StarRocksMvRefreshMode, StarRocksMvStorageEngine,
        StarRocksTableKind, StarRocksTableSnapshot, StarRocksTableState, StoredMaterializedView,
        StoredStarRocksDatabase, StoredStarRocksIndex, StoredStarRocksPartition,
        StoredStarRocksSchema, StoredStarRocksTable, StoredStarRocksTablet,
    };
    ...
    fn aggregate_full_refresh_executor_writes_physical_columns() {
        ...
        let profile = ObjectStoreProfile::from_s3_store_config(
            &state.starrocks_table_config.as_ref().expect("config").s3,
        )
    ...
    fn aggregate_mv_incremental_refresh_handles_base_delete() {
        ...
        // StarRocks table aggregate MV (count + sum grouped by customer),
        ...
        let Some((_config_dir, config_path)) = maybe_starrocks_table_config_path() else {
            return;
        };
    ...
    /// Read the StarRocks table aggregate MV's visible state as
    ...
    impl AfterCaptureHookGuard {
        fn install(hook: AfterCaptureHook) -> Self {
            ...
            crate::connector::starrocks::table::refresh_pin::clear_after_capture_hook();
            crate::connector::starrocks::table::refresh_pin::set_after_capture_hook(hook);
            Self {
    ...
    /// is unreachable (the caller skips). Mirrors `engine::tests::maybe_starrocks_table_config`.
    fn maybe_starrocks_table_config_path() -> Option<(tempfile::TempDir, std::path::PathBuf)> {
        ...
        let dir = tempfile::TempDir::new().expect("create StarRocks table config dir");
...
"#
    ...
    fn unique_s3_iceberg_warehouse(prefix: &str) -> String {
        let bucket = std::env::var("AWS_S3_BUCKET").unwrap_or_else(|_| "novarocks".to_string());
        let root = std::env::var("AWS_S3_ROOT")
    ...
    fn collect_current_snapshots_cleans_staged_partition_on_failure() {
        let (_dir, state, table_id) = seed_mv_refresh_state();
        let runtime = {
            let starrocks = state.starrocks_table.read().expect("StarRocks table");
            starrocks
    ...
    #[test]
    fn refresh_mv_rejects_full_for_starrocks_table_mv() {
        // REFRESH FULL is currently disabled for both StarRocks table and
        // iceberg-backed MVs pending redesign. This test locks in the
        // StarRocks table rejection path; the iceberg rejection is exercised
    ...
    fn seed_mv_refresh_state() -> (tempfile::TempDir, Arc<StandaloneState>, i64) {
        ...
        let table_id = {
            ...
            let created = starrocks_table_repo
                .create_table_layout(
                    txn.as_mut(),
                    crate::meta::repository::starrocks_table::CreateStarRocksTableLayoutRequest {
                        db_id: database.db_id,
                        table_name: "orders_mv".to_string(),
                        keys_type: "DUP_KEYS".to_string(),
                        bucket_num: 2,
                        kind: crate::meta::repository::starrocks_table::StarRocksTableKind::MaterializedView,
                        schema_version: 0,
                        tablet_schema_pb: crate::service::grpc_client::proto::starrocks::TabletSchemaPb::default()
                            .encode_to_vec(),
                        columns: vec![crate::meta::repository::starrocks_table::CreateStarRocksColumnRequest {
            ...
            mv_repo
                .create_definition_with_id(
                    ...
                    crate::meta::repository::mv::CreateMvDefinitionRequest {
                        ...
                        storage_engine: StarRocksMvStorageEngine::StarRocks
                            .as_sql_str()
                            .to_string(),
                        target_catalog: None,
        ...
        let read = provider.begin_read().expect("read");
        let snapshot = starrocks_table_repo
            .load_snapshot(read.as_ref())
    ...
    > {
        ...
        let snapshot = StarRocksTableSnapshot {
            ...
            materialized_views: vec![StoredMaterializedView {
                ...
                refresh_mode: StarRocksMvRefreshMode::DeferredManual,
                base_table_refs: vec![],
        ...
        for table in &snapshot.tables {
            put_seed_record(
                ...
                serde_json::json!({
                    ...
                    "state": starrocks_table_state_sql(table.state),
                    "kind": starrocks_table_kind_sql(table.kind),
                }),
            )?;
        }
        ...
        for partition in &snapshot.partitions {
            put_seed_record(
                ...
                serde_json::json!({
                    ...
                    "state": starrocks_partition_state_sql(partition.state),
                }),
            )?;
        }
        for index in &snapshot.indexes {
            put_seed_record(
                ...
                serde_json::json!({
                    ...
                    "state": starrocks_index_state_sql(index.state),
                }),
            )?;
        }
    ...
    fn starrocks_table_state_sql(state: StarRocksTableState) -> &'static str {
        match state {
    ...
    fn starrocks_table_kind_sql(kind: StarRocksTableKind) -> &'static str {
        match kind {
    ...
    fn starrocks_partition_state_sql(state: StarRocksPartitionState) -> &'static str {
        match state {
    ...
    fn starrocks_index_state_sql(state: StarRocksIndexState) -> &'static str {
        match state {
    ...
    fn maybe_object_store_config_for_mv_refresh() -> Option<StarRocksTableConfig> {
        let endpoint = std::env::var("AWS_S3_ENDPOINT")
            .unwrap_or_else(|_| "http://127.0.0.1:9000".to_string());
        if !starrocks_table_endpoint_reachable(&endpoint) {
            eprintln!(
