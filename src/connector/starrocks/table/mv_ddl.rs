...
mod tests {
    ...
    fn test_starrocks_table_config(
        warehouse_uri: String,
    ) -> super::super::config::StarRocksTableConfig {
        super::super::config::StarRocksTableConfig {
            warehouse_uri,
            s3: S3StoreConfig {
    ...
    fn state_with_inflight_starrocks_mv() -> (Arc<StandaloneState>, TempDir) {
        let dir = TempDir::new().expect("metadata tempdir");
        let metadata_path = dir.path().join("standalone.sqlite");
        let metadata_provider =
            crate::meta::SqliteMetaStoreProvider::open(&metadata_path).expect("open meta provider");
        let warehouse_uri = format!("file://{}", dir.path().join("starrocks").display());
        let config = test_starrocks_table_config(warehouse_uri.clone());
        ...
        let mut write = metadata_provider
            .begin_write("seed inflight StarRocks MV")
        ...
        let database =
            crate::meta::repository::starrocks_table::StarRocksTableMetaRepository::default()
                ...
                .expect("create StarRocks database");
        let created =
            crate::meta::repository::starrocks_table::StarRocksTableMetaRepository::default()
                .create_table_layout(
                    write.as_mut(),
                ...
                .expect("create StarRocks MV layout");
        crate::meta::repository::starrocks_txn::StarRocksTxnRepository::default()
            .record_visible_bootstrap(
                write.as_mut(),
                created.table.table_id,
                created.partition.partition_id,
    ...
    #[test]
    fn show_materialized_views_lists_iceberg_relationship_without_starrocks_table_row() {
    ...
    #[test]
    fn list_mv_rows_filters_starrocks_and_iceberg_storage_engines() {
        let (state, _dir) = state_with_inflight_starrocks_mv();
        insert_iceberg_mv_relationship(
            &state,
            "ice",
            "analytics",
    ...
    #[test]
    fn drop_starrocks_mv_preserves_repo_definition_when_legacy_drop_rejects() {
        // `state_with_inflight_starrocks_mv` builds a real StarRocks table
        ...
        let mv_id = state
            .starrocks_table
            .read()
            .expect("StarRocks table read lock")
        ...
        assert!(err.contains("inflight StarRocks txns"), "err={err}");
        assert!(
    ...
    #[test]
    fn starrocks_mv_partition_validation_rejects_iceberg_transforms() {
        ...
        let err = validate_starrocks_mv_partition_columns(Some(&fields), &output_columns)
            .expect_err("StarRocks table MV should not accept Iceberg partition transforms");
    ...
    fn count_distinct_shape() -> IncrementalMvShape {
        let stmt = parse_create_mv(
            "create materialized view mv1 distributed by hash(k) buckets 2 \
    ...
    #[test]
    fn aggregate_mv_analyzed_types_accepts_count_distinct_scalar_keys() {
        let shape = count_distinct_shape();
        for data_type in [
            DataType::Boolean,
            DataType::Int64,
            DataType::Float64,
            ...
            let resolved = resolved_count_distinct_query(data_type.clone());
            validate_incremental_mv_analyzed_types(&shape, &resolved).unwrap_or_else(|err| {
                panic!("COUNT(DISTINCT) key type {data_type:?} should be supported, got {err}")
            });
        }
    }

    #[test]
    fn aggregate_mv_analyzed_types_rejects_count_distinct_unsupported_keys() {
        let shape = count_distinct_shape();
...
GROUP BY k1",
        ...
        register_starrocks_table_in_catalog(&mut catalog, &runtime).expect("register mv");
    ...
    fn create_mv_routes_iceberg_storage_engine_to_phase4_path() {
        ...
        assert_eq!(resolved, StarRocksMvStorageEngine::Iceberg);
    }
