...
mod mv_target_tests {
    ...
    use crate::connector::starrocks::table::model::{
        ...
        StoredMaterializedView, StoredStarRocksDatabase, StoredStarRocksIndex,
    ...
    #[test]
    fn write_chunks_into_starrocks_partition_routes_rows_to_staged_tablets() {
    ...
    fn insert_plan_column_modes_split_visible_and_physical_columns() {
        ...
        let prepared = {
            ...
            let prepared = fixture
                ...
                .starrocks_txn_repo
                .prepare(
                    &fixture.state.starrocks_table_repo,
                    txn.as_mut(),
    ...
    fn visible_insert_plan_reports_hidden_distribution_keys() {
        ...
        assert_eq!(
            err,
            "StarRocks table analytics.orders_mv distribution key columns are hidden in visible insert mode; use physical insert plan",
        );
    ...
    fn mv_refresh_noop_chunks_updates_metadata_atomically() {
        ...
        let rows = write_chunks_into_starrocks_partition_for_mv_refresh(
            &fixture.state,
            plan,
            &[],
    ...
    fn stale_insert_plan_revalidates_visible_version_before_publish() {
        ...
        let rows = write_chunks_into_starrocks_partition(
            &fixture.state,
            stale_plan,
    ...
    fn aggregate_mv_upsert_replaces_existing_primary_key_row() {
        ...
        write_chunks_into_starrocks_partition(&fixture.state, old_plan, &old_chunks)
        ...
        let active_chunks =
            read_active_starrocks_physical_chunks(&fixture.state, &read_plan).expect("read active");
    ...
    fn aggregate_mv_pk_op_append_rejects_reserved_column_name() {
        let chunk = single_i32_chunk(STARROCKS_PK_OP_COLUMN, &[1]);
        let err = append_primary_key_op_column(&[chunk], STARROCKS_PK_OP_UPSERT)
    ...
    fn seed_repository_snapshot(
        ...
        let mut txn = provider
            .begin_write("seed StarRocks txn test repositories")
    ...
    fn seed_state_with_aggregate_primary_key_mv() -> MvTestFixture {
        ...
        let columns = vec![
            crate::connector::starrocks::table::model::StoredStarRocksColumn {
                schema_id: 100,
                ordinal: 0,
    ...
    enum MvFixtureStorage {
        Local,
        ObjectStore(StarRocksTableConfig),
    }
    ...
    fn maybe_object_store_config() -> Option<StarRocksTableConfig> {
