...
impl<'a> super::AnalyzerContext<'a> {
    ...
    fn analyze_function(
        ...
        // the resulting Arrow type to its StarRocks spelling. Other call
        // sites keep using Int64 for integer literals so the executor's
        // chunk widths stay consistent. Some function families return
        // BINARY/VARCHAR at the Arrow level but carry a distinct logical
        // type (BITMAP / HLL / JSON / null literal) in StarRocks, so
        // recognise those by the producing function name first.
...
/// (greatest / least / coalesce / nvl / ifnull). The original `kind`
/// is preserved so codegen sees the same shape; only `data_type` is
/// updated so `typeof()` can report the narrow spelling.
