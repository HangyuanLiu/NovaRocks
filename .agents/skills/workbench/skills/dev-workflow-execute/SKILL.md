...
1. 核实当前代码状态。
2. 实现最小的完整语义切片。
3. 运行覆盖本切片行为的定向测试或探针；按 approved plan 该任务的验证列执行，不用全量代替定向。
4. 修复失败并重跑。
...
迭代中默认只运行与本次改动相关的测试。相关性按 approved plan 该任务的验证列确定；plan 未覆盖的新增改动面，
按 contract 第 8 节现场界定，并把界定结果补回 plan 的执行记录。

全量验证只在 contract 第 8.1 节允许的点运行：wave / 里程碑收敛、声称实现完成前的最终验证，以及改动面无法
界定时（触及共享类型词汇表、wire 或持久化格式、跨模块公共契约、全局构建配置，或定向验证出现无法解释的失败）。
除此之外用全量代替定向不是更谨慎：它把单位改动的验证成本抬高一到两个数量级，并让机器满载。临时升级到全量前，
先记录触发条件。

满载的直接后果是负载敏感的假失败——依赖时间常量、超时或进程就绪的用例在并发压力下失败，与被改代码无关，并把
归因引向错误方向。因此：

1. 先看第一个失败，不看失败数量；一个早期失败可能让其后的用例集体成为级联受害者。
2. 判据是单独重跑该用例：单跑即过、且失败落在时间敏感用例上的，按负载噪声处理并在完成证据中记录，不据此回滚
   改动。
...
- plan 必需项全部完成；
- 运行与风险相称的格式、单元、集成、端到端和生产形态验证；最终验证点运行一次与改动面相称的全量；
- 检查 diff 与 accepted spec 一致；
- 记录未运行的测试及原因；
- 失败已按单跑判据区分真实失败与负载噪声，被判为噪声的用例及判据已记录；
