...
开篇简述本计划交付什么、消费哪个 spec 版本、当前实现到哪里。Spec 负责解释任务、目标和设计取舍；plan 重点说明
如何把已接受的设计接到现有代码，不重复抄写全部裁决。

Plan 默认保留以下六章及顺序，文档标题下的一级章节使用 `## 1. …`：

| 章节 | 本章职责 |
|---|---|
| 1. 实施目标与代码基线 | 指向 spec 及版本，说明当前状态、交付结果、代码基线与实施前提。 |
| 2. 关键数据结构与流程 | 展示核心结构、流程伪代码和交接，关联负责实现的阶段。 |
| 3. 实施阶段与依赖 | 提供具名 DAG 和每个阶段的完整执行说明。 |
| 4. 验证方案 | 写具体测试、负载、命令、通过条件与证据要求。 |
| 5. 实施边界与风险处理 | 说明局部自主选择、需重新裁决的变化、切换风险、检查点和恢复方式。 |
| 6. 执行记录 | 记录实际进度、检查点、验证结果与未完成事项，不预填通过。 |

一级章节默认保留，章内组织可调整；不适用时简短说明原因。用户或项目明确规定其他结构时遵循其要求。

对有状态、并发或跨组件交接的改动，给出关键数据结构与主流程伪代码，展示身份、可变状态和责任由谁持有、在哪里交接，
以及影响正确性的失败分支。标明哪些名字只是示意、哪些是已冻结接口，并关联负责实现的阶段。无需列出完整 API、
无关字段或逐行代码；简单改动可以用简短流程说明。

术语沿用 spec 的“名词解释”，保留可定位的入口；plan 新引入的内部对象在首次出现处说明职责、来源或示意状态。
不要把 spec 已裁决的关键机制重新留作实现任选项，也不要让示意类型名看起来像已存在的代码。

**实施阶段不可被精简掉。** 每个 DAG 中的 task ID 都有独立可定位的阶段说明，例如 `### P01：查询准入`，并保留四个
`####` 小节：`目标与前置输入`、`修改范围与关键工作`、`输出与交接`、`验证与完成条件`。执行者和调度约束放在前两节，
后继使用方式放在交接节；正文可以是短段落。单一串行任务不制造并行 wave；复杂计划保留真实依赖和调度信息。

阶段图用“编号 + 名称”，并说明依赖原因。任务多时可加紧凑索引表，但不能以表格或 `P00 → P01 → P02` 代替阶段正文；
精简重复文字时保留稳定 ID、交接和验收条件。公共验证命令可集中一次定义，各阶段明确引用对应范围，避免重复大段命令。
...
4. 把任务切成行为完整、可独立验证、文件所有权尽量不重叠的增量；不要机械按文件拆任务，也不要为了并行制造
   错误边界。
5. 按前述阶段说明写齐以下信息；它们是内容检查项，不要求逐条复制成字段：
   - 稳定 task ID；
   - `depends_on` 与调度 wave；
...
涉及性能验收时，除阈值外还需交代负载、输入、配置、测量入口和对照方法；尚需基线调查确定的参数，指定由哪个前置
阶段在测量前冻结、记录在哪里、后继如何复用，不能让验收执行者看到结果后选择通过条件。
...
- spec 和 plan 相互链接；
- 六章及每阶段四个小节可定位，结构例外有用户或项目依据；
- task DAG、并行 waves、文件所有权、调度标签、验收命令、生产形态验证和完成标准明确；
- 图中每个 task ID 都有具名阶段说明，前置、输出和验证可定位；关键结构与流程能对应到实施阶段；
