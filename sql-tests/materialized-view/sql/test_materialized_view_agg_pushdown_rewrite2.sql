-- Test Objective:
-- 1. Validate additional aggregate pushdown rewrite patterns on local tables.
-- 2. Cover non-trivial grouping and projection combinations.
-- Source: dev/test/sql/test_materialized_view/T/test_materialized_view_agg_pushdown_rewrite2
...
-- query 10
-- @result_contains_any=mv1
-- @result_contains_any=mv2
SET enable_materialized_view_rewrite = true;
EXPLAIN SELECT z.c_district AS c_district_name, CAST(SUM(x.c10) AS DOUBLE) / CAST(SUM(x.c11) AS DOUBLE) AS fare_per_km FROM t1 x LEFT JOIN (SELECT c1, MAX(c5) AS c_district FROM v1 GROUP BY c1) z ON CAST(z.c1 AS VARCHAR) = CAST(x.c4 AS VARCHAR) GROUP BY 1 ORDER BY 1;
