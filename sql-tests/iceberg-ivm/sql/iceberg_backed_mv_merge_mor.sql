...
-- @tags=write_path,mv,iceberg,ivm,storage_engine_iceberg,projection_filter,row_lineage,merge,mor
-- Test Point:
...
-- Method:
--   Create a projection MV, refresh once, MERGE updates +
...
PROPERTIES (
  ...
  "iceberg.catalog.warehouse" = "${iceberg_catalog_warehouse}/mv_merge_mor_${uuid0}",
...
INSERT INTO mv_merge_mor_ice_${uuid0}.ns_${uuid0}.staging VALUES
  (2, 'open', 25),
  (3, 'open', 30);
SET CATALOG mv_merge_mor_ice_${uuid0};
USE ns_${uuid0};
CREATE MATERIALIZED VIEW orders_merge_mor_mv
...
-- query 2
-- @skip_result_check=true
REFRESH MATERIALIZED VIEW orders_merge_mor_mv;

-- query 3
SELECT id, amount
FROM orders_merge_mor_mv
ORDER BY id;
...
-- query 8
-- @skip_result_check=true
DROP MATERIALIZED VIEW orders_merge_mor_mv;
