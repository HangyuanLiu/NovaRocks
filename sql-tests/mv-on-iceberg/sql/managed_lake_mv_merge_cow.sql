...
-- @tags=write_path,managed_lake,mv,iceberg,ivm,projection_filter,row_lineage,merge,cow
-- Test Point:
...
PROPERTIES (
  ...
  "iceberg.catalog.warehouse" = "${managed_lake_warehouse}/mv_merge_cow_${uuid0}",
...
INSERT INTO mv_merge_cow_ice_${uuid0}.ns_${uuid0}.staging VALUES
  (2, 'open', 25),
  (3, 'open', 30);
CREATE MATERIALIZED VIEW ${case_db}.orders_merge_cow_mv
...
-- query 2
-- @skip_result_check=true
REFRESH MATERIALIZED VIEW ${case_db}.orders_merge_cow_mv;

-- query 3
SELECT id, amount
FROM ${case_db}.orders_merge_cow_mv
ORDER BY id;
...
-- query 8
-- @skip_result_check=true
DROP MATERIALIZED VIEW ${case_db}.orders_merge_cow_mv;
