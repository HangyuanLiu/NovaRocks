...
-- @tags=runtime_filter,cross_process,distributed,dormancy
-- Test Objective:
...
-- 2. Prove Native binding lookup remains deliberately dormant: every scheduled
--    binding reports DeploymentNotInstalled and every data-plane side effect is zero.
...
CREATE TABLE ${case_db}.rf_dormant_partial_probe (
    id INT NOT NULL,
    k INT
)
TBLPROPERTIES ("format-version" = "3");

CREATE TABLE ${case_db}.rf_dormant_partial_build (
    k INT,
    flag VARCHAR(8)
)
TBLPROPERTIES ("format-version" = "3");
...
INSERT INTO ${case_db}.rf_dormant_partial_probe
...
INSERT INTO ${case_db}.rf_dormant_partial_build
...
ANALYZE TABLE ${case_db}.rf_dormant_partial_probe;
ANALYZE TABLE ${case_db}.rf_dormant_partial_build;
...
SELECT 'rf_off' AS mode, COUNT(*) AS row_count, COALESCE(SUM(p.id), 0) AS id_sum
FROM ${case_db}.rf_dormant_partial_probe p
JOIN ${case_db}.rf_dormant_partial_build b ON p.k = b.k
WHERE b.flag = 'Y';
...
SELECT 'rf_on' AS mode, COUNT(*) AS row_count, COALESCE(SUM(p.id), 0) AS id_sum
FROM ${case_db}.rf_dormant_partial_probe p
JOIN ${case_db}.rf_dormant_partial_build b ON p.k = b.k
WHERE b.flag = 'Y';
...
-- @result_contains=RuntimeFilterDormancy: lookups_observed=true all_deployment_not_installed=true zero_side_effects=true same_backend_partial_completion=true
