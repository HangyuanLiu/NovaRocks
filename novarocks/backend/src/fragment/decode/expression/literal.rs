...
//! Fragment literal expression lowering.
...
pub(crate) fn lower_literal(
    ...
    data_type: &DataType,
) -> Result<LiteralValue, super::NativeExpressionDecodeError> {
    lower_literal_at(literal, data_type).map_err(|error| {
        super::NativeExpressionDecodeError::from(
            error.into_protocol(FieldPath::root("expr").field("literal")),
        )
    })
}
...
pub(super) fn lower_literal_at(
    literal: &expr::LiteralExpr,
    data_type: &DataType,
) -> Result<LiteralValue, NativeExpressionLeafDecodeError> {
    let value = literal.value.as_ref().ok_or_else(|| {
        NativeExpressionLeafDecodeError::at_field(
...
) -> NativeExpressionLeafDecodeError {
    NativeExpressionLeafDecodeError::at_field(kind, "value", detail).append_field(variant)
}
...
fn require_literal_type(
    ...
    variant: &'static str,
    context: &str,
) -> Result<(), NativeExpressionLeafDecodeError> {
    if ok {
        Ok(())
    } else {
...
fn lower_decimal_literal(
    ...
    match data_type {
        ...
        _ => Err(NativeExpressionLeafDecodeError::at_field(
            ProtocolErrorKind::InvalidValue,
            "value",
...
fn validate_decimal_parts(precision: u8, scale: i8) -> Result<(), NativeExpressionLeafDecodeError> {
    if precision == 0 || precision > 76 {
        return Err(NativeExpressionLeafDecodeError::at_field(
            ProtocolErrorKind::OutOfRange,
            "precision",
...
fn decimal_bytes<const N: usize>(
    value: &[u8],
    label: &str,
) -> Result<[u8; N], NativeExpressionLeafDecodeError> {
...
mod tests {
    ...
    fn literal_error(
        literal: expr::LiteralExpr,
        data_type: DataType,
    ) -> super::super::NativeExpressionDecodeError {
