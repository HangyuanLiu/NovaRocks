...
use super::{
    ConnectorClusterIdentity, ConnectorError, ConnectorErrorKind, ConnectorExecutionBindingKey,
    ConnectorExternalFenceFailure, ConnectorExternalFenceGeneration, ConnectorInstanceDescriptor,
    ConnectorInstanceIncarnation, ConnectorRequestContext, ConnectorTableIdentity, CreatePolicy,
    MAX_CONNECTOR_EXTERNAL_FENCE_IDENTITY_BYTES,
};

pub const CONNECTOR_CTAS_STAGED_PUBLICATION_CONTRACT_VERSION: u32 = 1;
pub const MAX_CONNECTOR_CTAS_PUBLICATION_PAYLOAD_BYTES: usize = 64 * 1024;
...
impl ConnectorCtasOperationId {
    pub fn try_from_bytes(bytes: [u8; 16]) -> Result<Self, ConnectorError> {
        if bytes == [0; 16] {
            return Err(invalid("connector CTAS operation ID must be set"));
        }
        Ok(Self(bytes))
    }
...
impl ConnectorCtasActionId {
    pub fn try_from_bytes(bytes: [u8; 16]) -> Result<Self, ConnectorError> {
        if bytes == [0; 16] {
            return Err(invalid("connector CTAS action ID must be set"));
        }
        Ok(Self(bytes))
    }
...
impl ConnectorCtasPublicationFenceReceipt {
    pub fn try_new(
        ...
        let digest = opaque_digest(FENCE_RECEIPT_DOMAIN, &[&fence.digest()], &payload);
        Ok(Self {
    ...
    pub fn validate_for(
        ...
        if self.fence_digest != fence.digest() {
            return Err(foreign(
                "CTAS publication fence receipt answers another fence",
            ));
        }
        let expected = opaque_digest(FENCE_RECEIPT_DOMAIN, &[&self.fence_digest], &self.payload);
...
impl ConnectorCtasProofPurpose {
    fn for_publish(disposition: ConnectorCtasPublishDisposition) -> Self {
        match disposition {
            ...
            ConnectorCtasPublishDisposition::Conflict => Self::PublishConflict,
        }
    }
    ...
    fn for_abort(disposition: ConnectorCtasAbortDisposition) -> Self {
        match disposition {
            ...
            ConnectorCtasAbortDisposition::AlreadyPublished => Self::AbortAlreadyPublished,
            ConnectorCtasAbortDisposition::Conflict => Self::AbortConflict,
        }
    }
...
impl ConnectorCtasAdvanceFenceRequest {
    pub fn try_new(
        ...
        fence.validate()?;
        let input_digest = aggregate_result_digest(ADVANCE_REQUEST_DOMAIN, &[&fence.digest()]);
        Ok(Self {
            fence,
            input_digest,
            context,
    ...
    pub fn validate(&self) -> Result<(), ConnectorError> {
        self.fence.validate()?;
        let expected = aggregate_result_digest(ADVANCE_REQUEST_DOMAIN, &[&self.fence.digest()]);
...
impl ConnectorCtasStageRequest {
    ...
    pub fn try_new(
        ...
        fence.validate()?;
        require_digest("CTAS staged target", target_digest)?;
        require_digest("CTAS initialization", initialization_digest)?;
        validate_payload_allow_empty("CTAS stage provider payload", &provider_payload)?;
        let input_digest = stage_request_digest(
            &owner,
            &fence,
            action_id,
            target_digest,
    ...
    pub fn validate_for(&self, owner: &ConnectorExecutionBindingKey) -> Result<(), ConnectorError> {
        ...
        let expected = stage_request_digest(
            &self.owner,
            &self.fence,
            self.action_id,
            self.target_digest,
...
impl ConnectorCtasPublishRequest {
    ...
    pub fn try_new(
        ...
        let input_digest = publish_request_digest(
            &owner,
            &fence,
            action_id,
            &locator,
    ...
    pub fn validate_for(&self, owner: &ConnectorExecutionBindingKey) -> Result<(), ConnectorError> {
        ...
        let expected = publish_request_digest(
            &self.owner,
            &self.fence,
            self.action_id,
            &self.locator,
...
impl ConnectorCtasAbortRequest {
    pub fn try_new(
        ...
        let input_digest = abort_request_digest(&owner, &fence, action_id, &locator, &proof);
        Ok(Self {
            owner,
            fence,
            action_id,
    ...
    pub fn validate_for(&self, owner: &ConnectorExecutionBindingKey) -> Result<(), ConnectorError> {
        ...
        let expected = abort_request_digest(
            &self.owner,
            &self.fence,
            self.action_id,
            &self.locator,
...
impl ConnectorHistoricalCtasDescriptor {
    ...
    pub fn try_new(
        ...
        let digest = historical_descriptor_digest(
            &historical_binding,
            &fence,
...
impl ConnectorHistoricalCtasObservation {
    pub fn try_new(
        ...
        let digest = historical_observation_digest(
            &inspection_binding,
            descriptor.digest,
...
impl ConnectorHistoricalCtasCleanupRequest {
    pub fn validate(&self) -> Result<(), ConnectorError> {
        ...
        if !self.observation.disposition.may_cleanup() || self.observation.locator.is_none() {
            return Err(invalid(
...
pub trait ConnectorCtasStagedPublication: Send + Sync {
    ...
    ) -> Result<ConnectorCtasPublicationFenceReceipt, ConnectorError>;

    fn stage(
        &self,
        request: ConnectorCtasStageRequest,
    ) -> Result<ConnectorCtasStageResult, ConnectorError>;

    fn publish(
        &self,
        request: ConnectorCtasPublishRequest,
    ) -> Result<ConnectorCtasPublishResult, ConnectorError>;

    fn abort(
        &self,
        request: ConnectorCtasAbortRequest,
    ) -> Result<ConnectorCtasAbortResult, ConnectorError>;
}
...
pub trait ConnectorHistoricalCtasStagedPublicationRecovery: Send + Sync {
    ...
    fn inspect(
        ...
        descriptor: ConnectorHistoricalCtasDescriptor,
        context: ConnectorRequestContext,
    ) -> Result<ConnectorHistoricalCtasObservation, ConnectorError>;

    fn cleanup(
        &self,
        request: ConnectorHistoricalCtasCleanupRequest,
    ) -> Result<ConnectorHistoricalCtasCleanupReceipt, ConnectorError>;
}
...
impl ConnectorCtasStagedPublicationLease {
    ...
    pub fn advance_fence(
        ...
        receipt.validate_for(&request.fence)?;
        Ok(receipt)
    }
    ...
    pub fn stage(
        ...
        request.validate_for(&self.owner)?;
...
#[allow(clippy::too_many_arguments)]
fn historical_descriptor_digest(
    historical_binding: &ConnectorExecutionBindingKey,
    fence: &ConnectorCtasPublicationFence,
    fence_receipt_digest: [u8; 32],
    target_digest: [u8; 32],
    ...
    hasher.update([create_policy as u8]);
    hasher.update(locator.map(|value| value.digest).unwrap_or_default());
    for checkpoint in checkpoints {
        hasher.update(checkpoint.action_id.to_bytes());
        hasher.update([checkpoint.action as u8, checkpoint.dispatch as u8]);
        hasher.update(checkpoint.input_digest);
        hasher.update(checkpoint.evidence_digest.unwrap_or_default());
    }
    hasher.update(evidence.map(|value| value.digest).unwrap_or_default());
    hasher.finalize().into()
}

fn historical_observation_digest(
    inspection_binding: &ConnectorExecutionBindingKey,
    ...
    hasher.update(inspection_binding.instance_id.as_str().as_bytes());
...
mod tests {
    ...
    fn fence(attempt: u64) -> ConnectorCtasPublicationFence {
        ConnectorCtasPublicationFence::try_new(
            ...
            generation(attempt),
            ConnectorCtasOperationId::try_from_bytes([9; 16]).unwrap(),
            target("orders"),
        )
        .unwrap()
    }
    ...
    fn action(value: u8) -> ConnectorCtasActionId {
        ConnectorCtasActionId::try_from_bytes([value; 16]).unwrap()
    }
    ...
    fn fence_orders_only_one_catalog_authority() {
        ...
        let foreign = ConnectorCtasPublicationFence::try_new(
            ConnectorClusterIdentity::derive("cluster-a").unwrap(),
            generation(2),
            ConnectorCtasOperationId::try_from_bytes([8; 16]).unwrap(),
            target("orders"),
        )
        .unwrap()
        .validate_monotonic_successor_of(&established)
    ...
    fn opaque_values_are_bounded_and_digest_sealed() {
        ...
        let receipt =
            ConnectorCtasPublicationFenceReceipt::try_new(&fence, Bytes::from_static(b"receipt"))
                .unwrap();
        receipt.validate_for(&fence).unwrap();
    ...
    fn result_aggregates_seal_dispositions_locators_and_proofs() {
        ...
        .unwrap();
        publish.disposition = ConnectorCtasPublishDisposition::Conflict;
        assert_eq!(
            publish
        ...
        .unwrap();
        abort.disposition = ConnectorCtasAbortDisposition::AlreadyPublished;
        assert_eq!(
            abort
                .validate_for(&abort_request)
                .unwrap_err()
                .external_fence_failure(),
            Some(ConnectorExternalFenceFailure::ForeignOperation)
        );
    ...
    fn all_historical_dispositions_are_typed_and_fail_closed() {
        ...
        for disposition in [
            ...
            let locator = if disposition == ConnectorHistoricalCtasDisposition::Staged {
                staged_locator.clone()
            } else {
                None
            };
            let inspection_binding = binding(2);
            let observation_proof = proof(
