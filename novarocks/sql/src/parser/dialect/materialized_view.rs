...
use crate::parser::dialect::create_table::parse_partition_field_expr;
...
mod tests {
    ...
    fn parse_create_mv_rejects_refresh_async_without_trigger() {
        let err = crate::parser::parse_sql(
            "CREATE MATERIALIZED VIEW mv1 \
    ...
    fn parse_drop_mv_rejects_force() {
        let err = crate::parser::parse_sql("DROP MATERIALIZED VIEW mv1 FORCE")
            .expect_err("should reject");
    ...
    fn parse_alter_mv_rejects_refresh_immediate() {
        let err = crate::parser::parse_sql("ALTER MATERIALIZED VIEW mv1 SET REFRESH IMMEDIATE")
            .expect_err("should reject");
        assert!(
    ...
    fn parse_refresh_mv_rejects_async_modifier() {
        let err = crate::parser::parse_sql("REFRESH MATERIALIZED VIEW mv1 WITH ASYNC MODE")
            .expect_err("should reject");
        assert!(
    ...
    fn parse_show_materialized_views_rejects_like_and_where() {
        let err_like = crate::parser::parse_sql("SHOW MATERIALIZED VIEWS LIKE '%mv%'")
        ...
        let err_where = crate::parser::parse_sql("SHOW MATERIALIZED VIEWS WHERE name = 'mv1'")
    ...
    fn parse_create_materialized_view_keeps_storage_engine_property() {
        ...
        let crate::parser::ast::Statement::CreateMaterializedView(create) = stmt else {
            panic!("expected CREATE MATERIALIZED VIEW");
        };
        assert_eq!(
            create.properties,
