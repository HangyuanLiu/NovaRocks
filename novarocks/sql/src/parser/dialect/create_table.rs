...
use crate::analyzer::iceberg_ref::split_ref_suffix;
...
pub(crate) fn parse_default_literal(
...
) -> Result<crate::parser::ast::DefaultLiteral, String> {
    use crate::parser::ast::DefaultLiteral;
    use novarocks_catalog::schema::SqlType;
    ...
    crate::literal::default_literal_to_column_default(&lit, data_type)?;
...
fn parse_string_default(
    ...
    match data_type {
        ...
        SqlType::Date => {
            let days = crate::literal::parse_date_string_to_days(s)?;
            Ok(DefaultLiteral::Date(days))
        }
        SqlType::DateTime => {
            let micros = crate::literal::parse_datetime_string_to_micros(s)?;
            Ok(DefaultLiteral::DateTime(micros))
        }
        SqlType::DateTimeNs => {
            let nanos = crate::literal::parse_datetime_string_to_nanos(s)?;
...
mod tests {
    ...
    fn parse_create_table_accepts_nested_array_complex_columns() {
        ...
        let normalized =
            crate::parser::dialect::normalize_for_raw_parse(sql).expect("normalize should succeed");
        let mut parser = sqlparser::parser::Parser::new(&StarRocksDialect)
            .try_with_sql(&normalized)
            .expect("build parser");
        let stmt = parse_create_table_statement(&mut parser);
    ...
    fn parse_create_table_captures_int_default() {
        ...
        assert_eq!(
            columns[1].default,
            Some(crate::parser::ast::DefaultLiteral::Int(5))
        );
    }
    ...
    fn parse_create_table_captures_default_null() {
        ...
        assert_eq!(
            columns[1].default,
            Some(crate::parser::ast::DefaultLiteral::Null)
        );
    }
