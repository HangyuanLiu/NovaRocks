...
fn normalize_branch_row_id_output(
    plan: &mut LogicalPlanNode,
    row_id_column: crate::column_id::ColumnId,
) -> Result<(), String> {
    let LogicalPlanKind::Project(project) = &mut plan.kind else {
        return Err(
...
fn branch_output_action_column_id(plan: &LogicalPlanNode) -> Option<crate::column_id::ColumnId> {
    match &plan.kind {
        LogicalPlanKind::Scan(scan) => scan
            .columns
            .iter()
...
mod tests {
    ...
    use crate::analysis::{JoinKind, LiteralValue};
