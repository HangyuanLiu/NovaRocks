...
use crate::planner::runtime_filter::graph::RuntimeFilterGraph;
use crate::planner::runtime_filter::sealed::SealedRuntimeFilterPlan;
...
impl DistributedPlan {
    ...
    pub(crate) fn runtime_filter_join_progress(
        &self,
    ) -> &crate::planner::runtime_filter::progress::JoinBuildProgressCatalog {
...
pub(in crate::planner::distributed) enum DistributedPlanSealError {
    EmptyFragments,
...
pub(in crate::planner::distributed) fn seal_draft(
...
pub(super) mod test_support {
    ...
    use crate::planner::distributed::fragment::{
        DataPartition, DataSink, DistributedPlanDraft, PlanFragment,
    };
    use crate::planner::distributed::node::{DistributedNode, DistributedNodeKind};
...
mod tests {
    ...
    use crate::analysis::{ExprKind, JoinKind, LiteralValue, OutputColumn, TypedExpr};
    use crate::column_id::ColumnId;
    use crate::planner::distributed::fragment::{
        DataPartition, DataSink, DistributedPlanDraft, FragmentEdge, FragmentEdgeKind,
        FragmentStreamKind, PlanFragment,
    };
    use crate::planner::distributed::node::{
        DistributedNode, DistributedNodeKind, ExchangeFlavor, ExchangeReceiver,
    };
    use crate::planner::distributed::validation::RuntimeFilterPlanValidationError;
    ...
    use crate::planner::runtime_filter::activation::{ActivationConstraint, ActivationFallback};
    ...
    fn join_consumer_binding(
        ...
        RuntimeFilterBindingSpecData {
            ...
            role: RuntimeFilterBindingRoleData::Consumer(ConsumerRequirementData {
                ...
                target:
                    crate::planner::runtime_filter::graph::ConsumerBindingTarget::SourceBoundary {
                        scan_domain: None,
                    },
            }),
        }
    ...
    fn seal_downgrades_only_graph_consumer_for_valid_pure_cycle() {
        ...
        assert_eq!(
            ...
            ConsumerActivation::NonBlockingLive {
                late_apply: crate::planner::runtime_filter::contract::LateApplyGranularity::Batch,
            }
        );
        ...
        assert!(matches!(
            ...
            crate::planner::runtime_filter::activation::ActivationDecisionReason::CycleForced { .. }
        ));
    ...
    fn seal_catalog_covers_each_consumer_in_binding_id_order() {
        ...
        for decision in decisions.values() {
            ...
            assert!(matches!(
                &decision.reason,
                crate::planner::runtime_filter::activation::ActivationDecisionReason::ConservativeFallback
            ));
        }
    }
    ...
    fn seal_reports_the_structural_error_before_the_runtime_filter_graph_error() {
        ...
        let mut draft = super::test_support::single_fragment_draft(Some(0));
        draft.fragments[0].sink = crate::planner::distributed::fragment::DataSink::Noop;
        draft.runtime_filter_graph = graph_with_binding_to_unknown_channel();
