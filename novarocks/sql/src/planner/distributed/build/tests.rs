...
struct ActivationNeutralBindingStructure {
    ...
    location: crate::planner::runtime_filter::graph::PlanLocation,
    expression: String,
    apply_point: crate::planner::runtime_filter::graph::ApplyPoint,
...
fn aggregate_topn_runtime_filter_materializes_ordered_live_graph() {
    ...
    crate::planner::distributed::validation::validate_runtime_filter_graph_against_plan(
        graph,
...
fn aggregate_topn_runtime_filter_keeps_ordered_bound_contract_live_in_draft() {
    ...
    assert_eq!(
        ...
        ActivationConstraint::LiveOnly {
            late_apply: crate::planner::runtime_filter::contract::LateApplyGranularity::Batch,
            reason:
                crate::planner::runtime_filter::activation::RequiredLiveReason::OrderedBoundContract,
        }
    );
}
...
fn build_distributed_plan_keeps_runtime_filter_probe_on_filter() {
    ...
    assert_eq!(
        requirement.target,
        crate::planner::runtime_filter::graph::ConsumerBindingTarget::DirectInput {
            input_ordinal: 0,
        }
    );
    assert_column_ref(&binding.expression, 1, "l_k");
...
fn draft_runtime_filter_population_preserves_join_structure_without_sealed_activation() {
    ...
    assert_snapshot_detects_binding_mutation(&sealed_graph, "location", |binding| {
        if matches!(&binding.role, RuntimeFilterBindingRole::Producer(_)) {
            binding.location.node_id =
                crate::planner::runtime_filter::contract::PlanNodeId::new(99);
        }
    });
    ...
    assert_snapshot_detects_binding_mutation(&sealed_graph, "apply_point", |binding| {
        if matches!(&binding.role, RuntimeFilterBindingRole::Producer(_)) {
            binding.apply_point = crate::planner::runtime_filter::graph::ApplyPoint::NodeInput;
        }
    });
    ...
    let draft_consumer = draft_graph
        .bindings()
        .find_map(|binding| match &binding.role {
            crate::planner::runtime_filter::graph::RuntimeFilterBindingRoleData::Consumer(
                consumer,
            ) => Some(consumer),
            crate::planner::runtime_filter::graph::RuntimeFilterBindingRoleData::Producer(_) => {
                None
            }
        })
...
fn build_distributed_plan_hash_redistribute_creates_exchange_edge() {
    ...
    assert!(matches!(
        ...
        crate::planner::distributed::ExchangeFlavor::Distribution
    ));
...
fn stats_with_row_count_and_cost(output_row_count: f64) -> PhysicalPlanStats {
    PhysicalPlanStats {
        ...
        broadcast_decision: Some(crate::planner::physical::PlannerBroadcastDecision {
            feasible: true,
            forced: false,
            build_bytes: 10.0,
...
fn distributed_hash_join_node_with_exchanges(
    ...
    let exchange = |node_id, source_fragment_id| DistributedNode {
        ...
        children: vec![],
        stats: stats(),
        payload: DistributedNodeKind::Exchange(crate::planner::distributed::ExchangeReceiver {
