...
fn classify_scalar_operand(
    arena: &ScalarArena,
    expr: ScalarId,
    left_ids: &HashSet<crate::column_id::ColumnId>,
    right_ids: &HashSet<crate::column_id::ColumnId>,
    ...
    if *column_id != crate::column_id::ColumnId::UNSET {
...
mod column_id_helper_tests {
    ...
    fn project_passthrough_and_computed_both_collected() {
        use crate::analysis::{BinOp, ProjectItem, TypedExpr};
        ...
        let computed_item = ProjectItem {
            expr: TypedExpr {
                kind: crate::analysis::ExprKind::BinaryOp {
    ...
    fn generate_series_output_id_is_collected() {
        use crate::planner::payload::PlanGenerateSeriesNode;
    ...
    fn test_join_equi_keys_opt(
        ...
        let mut arena = ScalarArena::new();
        let expr =
            crate::planner::optimizer_bridge::logical::to_optimizer_expr(join_plan, &mut arena);
    ...
    fn join_equi_keys_disambiguates_self_join_by_qualifier() {
        ...
        let join = LogicalPlanNode::new(
            LogicalPlanKind::Join(LogicalJoinNode {
                join_type: crate::analysis::JoinKind::LeftSemi,
