...
use crate::optimizer::scalar::{ScalarArena, ScalarId, ScalarNode, SortKey};
...
mod tests {
    ...
    fn cmp(left: TypedExpr, op: crate::analysis::BinOp, right: TypedExpr) -> TypedExpr {
        TypedExpr {
    ...
    fn test_scan_source() -> ScanSource {
        crate::compiler::mv_rewrite::test_scan_source(
    ...
    fn join_plan(
        ...
        LogicalPlanNode::new(
            LogicalPlanKind::Join(crate::planner::logical::LogicalJoinNode {
                join_type: kind,
                condition: on,
            }),
            vec![left, right],
    ...
    fn extracts_inner_join_two_tables() {
        ...
        let c = col(3, "c");
        let d = col(4, "d");
        let on = cmp(col_ref(&a), crate::analysis::BinOp::Eq, col_ref(&c));
        let plan = join_plan(
            JoinKind::Inner,
    ...
    fn extracts_inner_join_with_driving_filter() {
        ...
        let driving = LogicalPlanNode::new(
            LogicalPlanKind::Filter(PlanFilterNode {
                predicate: cmp(col_ref(&a), crate::analysis::BinOp::Ge, int_lit(5)),
            }),
    ...
    fn rejects_inner_join_with_only_residual_condition() {
        ...
        let a = col(1, "a");
        let c = col(3, "c");
        let on = cmp(col_ref(&a), crate::analysis::BinOp::Gt, col_ref(&c));
        let plan = join_plan(
            JoinKind::Inner,
            scan_plan_named("t1", std::slice::from_ref(&a)),
            scan_plan_named("t2", std::slice::from_ref(&c)),
    ...
    fn rejects_inner_join_with_only_same_side_equality() {
        ...
        let b = col(2, "b");
        let c = col(3, "c");
        let on = cmp(col_ref(&a), crate::analysis::BinOp::Eq, col_ref(&b));
        let plan = join_plan(
            JoinKind::Inner,
            scan_plan_named("t1", &[a.clone(), b.clone()]),
            scan_plan_named("t2", std::slice::from_ref(&c)),
    ...
    fn rejects_right_deep_join() {
        ...
        let c = col(3, "c");
        let e = col(5, "e");
        let on_inner = cmp(col_ref(&c), crate::analysis::BinOp::Eq, col_ref(&e));
        let inner = join_plan(
            JoinKind::Inner,
            scan_plan_named("t2", std::slice::from_ref(&c)),
        ...
        let on_outer = cmp(col_ref(&a), crate::analysis::BinOp::Eq, col_ref(&c));
        let plan = join_plan(
            JoinKind::Inner,
            scan_plan_named("t1", std::slice::from_ref(&a)),
            inner,
    ...
    fn memo_root(plan: &LogicalPlanNode) -> (crate::optimizer::memo::Memo, MExpr) {
        let mut memo = crate::optimizer::memo::Memo::new();
    ...
    fn from_memo_rejects_split_aggregate() {
        use crate::optimizer::operator::{AggregateOutputLayout, LogicalAggregateOp, Operator};
