...
pub(crate) fn is_known_rule_name(name: &str) -> bool {
    cascades_rules::all_transformation_rules()
        ...
        || name == crate::planner::physical::runtime_filter_placement::RUNTIME_FILTER_RULE
...
mod is_known_rule_name_tests {
    ...
    use crate::planner::optimizer_bridge::logical::try_to_optimizer_expr;
    ...
    fn scalar_cast_int64(
        arena: &mut ScalarArena,
        child: crate::optimizer::scalar::ScalarId,
    ) -> crate::optimizer::scalar::ScalarId {
        arena.intern(
            ScalarNode::Cast {
                child,
                    ...
                    (
                        ...
                        BaseColumnStatistics {
                            ...
                            ndv: StatValue::missing(
                                crate::optimizer::stats_input::StatsMissingReason::ColumnNotReported(
                                    column.to_string(),
                                ),
                            ),
                        },
    ...
    fn plan_contains_mv_scan(node: &OptimizedOperatorNode, mv_name: &str) -> bool {
        match &node.op {
    ...
    fn optimizer_selects_cheaper_exact_or_mv_candidate() {
        ...
        let query_predicate = {
            ...
            let left = scalar_cmp(&mut query_scalars, query_a, crate::common::BinOp::Gt, 10);
            let right = scalar_cmp(&mut query_scalars, query_b, crate::common::BinOp::Lt, 3);
            scalar_or(&mut query_scalars, left, right)
        };
        let scan = OptExpr::leaf(Operator::LogicalScan(crate::optimizer::operator::ScanOp {
        ...
        let project_items = [base_a.clone(), base_b.clone()]
            .into_iter()
            .map(|column| crate::optimizer::operator::ScalarProjectItem {
        ...
        let mv_predicate = {
            ...
            let left = scalar_cmp(&mut mv_scalars, mv_a_ref, crate::common::BinOp::Gt, 10);
            let right = scalar_cmp(&mut mv_scalars, mv_b_ref, crate::common::BinOp::Lt, 3);
            scalar_or(&mut mv_scalars, left, right)
        };
        let mv_scan = OptExpr::leaf(Operator::LogicalScan(crate::optimizer::operator::ScanOp {
            database: "ns".to_string(),
        ...
        let mv_expr = OptExpr::new(
            Operator::LogicalFilter(crate::optimizer::operator::FilterOp {
    ...
    fn optimize_implements_assert_one_row() {
        ...
        use crate::column_id::ColumnRefFactory;
        use crate::planner::logical::{LogicalPlanKind, LogicalPlanNode};
        use crate::planner::payload::{PlanAssertOneRowNode, PlanValuesNode};
    ...
    fn optimize_rejects_residual_apply_when_rule_disabled() {
        ...
        let settings = crate::optimizer::options::SessionOptimizerSettings {
            disabled_rules: vec![
    ...
    fn assert_root_distribution(
        optimized_tree: &crate::optimizer::optimized_tree::OptimizedOperatorNode,
        expected: &crate::optimizer::property::DistributionSpec,
    ) {
        match &optimized_tree.op {
            crate::optimizer::operator::Operator::PhysicalDistribution(op) => {
    ...
    fn optimize_preserves_ranking_window_partition_topn_sort() {
        ...
        fn logical_has_rank_partition_topn_sort(
            ...
            if let crate::planner::logical::LogicalPlanKind::Sort(sort) = &plan.kind
                && sort.partition_limit == Some(2)
                && sort.topn_type == Some(SqlTopNType::Rank)
            {
                return true;
        ...
        let mut rewrite_ctx =
            crate::optimizer::rewrite::context::RewriteContext::for_query(Vec::new());
        ...
        let rewritten_expr = crate::optimizer::rewrite::registry::query_rewrite_pipeline()
            .rewrite(opt_plan, &mut rewrite_ctx)
            .expect("rewrite pipeline");
        let rewritten_logical = crate::planner::optimizer_bridge::logical::to_logical_plan(
            rewritten_expr,
            &arena.borrow(),
        );
        assert!(
