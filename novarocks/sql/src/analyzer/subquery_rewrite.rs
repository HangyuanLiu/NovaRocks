...
impl<'a> AnalyzerContext<'a> {
    ...
    fn collect_scalar_apply_spec(
        ...
        let replacement = crate::analysis::TypedExpr {
...
fn collect_outer_ref_column_ids(
    ...
    inner_scope: &AnalyzerScope,
    outer_scope: &AnalyzerScope,
    out: &mut Vec<crate::column_id::ColumnId>,
) {
    match &expr.kind {
...
fn collect_subquery_outer_ref_usage(
    resolved_sub: &crate::analysis::ResolvedQuery,
    inner_scope: &AnalyzerScope,
    outer_scope: &AnalyzerScope,
...
fn locate_scalar_placeholder_clause(
    ...
    placeholder_id: usize,
) -> Option<crate::analysis::ApplyClause> {
    if select
        .filter
        .as_ref()
        ...
        .unwrap_or(false)
    {
        return Some(crate::analysis::ApplyClause::Where);
    }
    if select
        .having
        .as_ref()
        ...
        .unwrap_or(false)
    {
        return Some(crate::analysis::ApplyClause::Having);
    }
    if select
        .projection
        .iter()
        .any(|p| expr_contains_placeholder(&p.expr, placeholder_id))
    {
        ...
        {
            return Some(crate::analysis::ApplyClause::AggregateInput);
        }
        return Some(crate::analysis::ApplyClause::Projection);
    }
    None
}
...
fn collect_correlation_column_ids(
    ...
    let mut ids: Vec<crate::column_id::ColumnId> = Vec::new();
    for pred in &preds {
...
fn collect_column_ids_in_expr(expr: &TypedExpr, out: &mut Vec<crate::column_id::ColumnId>) {
    match &expr.kind {
        ExprKind::ColumnRef { column_id, .. } => {
            out.push(*column_id);
        }
