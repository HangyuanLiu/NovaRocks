...
use crate::common::ScanVariantColumn;
use crate::explain::{
...
fn format_sort_node(
    ...
    if let Some(limit) = sort.partition_limit {
        let topn_type = match sort.topn_type {
            Some(crate::common::SqlTopNType::RowNumber) => "ROW_NUMBER",
            Some(crate::common::SqlTopNType::Rank) => "RANK",
            Some(crate::common::SqlTopNType::DenseRank) => "DENSE_RANK",
            None => "ROW_NUMBER",
        };
...
fn format_set_op_node(
    ...
    let label = match set_op.kind {
        ...
        SetOpKind::UnionDistinct => {
            unreachable!(
                "{}",
                crate::planner::distributed::build::union_distinct_must_be_rewritten_error()
            )
        }
...
mod tests {
    ...
    use crate::analysis::{ExprKind, JoinKind, OutputColumn, ProjectItem, SortItem, TypedExpr};
    use crate::column_id::ColumnId;
    use crate::common::SqlTopNType;
    use crate::explain::distributed::explain_distributed_plan;
    use crate::explain::{ExplainLevel, PlanNodeExplainStage};
    ...
    use crate::optimizer::optimized_tree::{
    ...
    use crate::optimizer::statistics::{ColumnStatistic, Confidence, CostEstimate, Statistics};
    use crate::planner::distributed::{
    ...
    fn test_broadcast_decision() -> crate::optimizer::cost::BroadcastDecision {
        crate::optimizer::cost::BroadcastDecision {
            feasible: true,
            forced: false,
    ...
    fn normal_scan_project_agg_renders_node_id_prefixes() {
        ...
        let physical_plan = crate::planner::optimizer_bridge::to_physical_plan(&optimized_tree)
            .expect("convert optimizer physical plan");
        let distributed_plan = crate::planner::pipeline::build_distributed_plan(physical_plan)
    ...
    fn shared_distributed_formatter_path_covers_scan_and_project() {
        ...
        let root_kind = crate::planner::distributed::distributed_kind_to_physical(&root.payload);
        assert_eq!(
        ...
        let scan_kind =
            crate::planner::distributed::distributed_kind_to_physical(&root.children[0].payload);
        assert_eq!(
            format_distributed_shared_plan_node_header(
                &scan_kind,
                PlanNodeExplainStage::Distributed
    ...
    fn distributed_explain_accepts_exchange_only_in_distributed_kind() {
        let node = DistributedNode {
            ...
            payload: DistributedNodeKind::Exchange(ExchangeReceiver {
                partition: crate::planner::distributed::DataPartition::unpartitioned(),
    ...
    fn detailed_ir_explain_shows_aggregate_topn_runtime_filter_contract() {
        ...
        let join_physical =
            crate::planner::optimizer_bridge::to_physical_plan(&join).expect("convert join plan");
        let join_distributed = crate::planner::pipeline::build_distributed_plan(join_physical)
    ...
    fn sort_with_partition_limit_plan(child: OptimizedOperatorNode) -> OptimizedOperatorNode {
        ...
        physical_node_with_scalars(
            Operator::PhysicalSort(crate::optimizer::operator::SortOp {
                items: intern_sort_items(
                    &mut scalars,
                    &[SortItem {
                        expr: column_ref_expr(1, "k", DataType::Int64, false),
