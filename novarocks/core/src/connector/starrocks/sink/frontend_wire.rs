...
mod tests {
    ...
    use crate::thrift::{descriptors, exprs, frontend_service, status, status_code, types};
    mod fe_rpc_server {
        include!(concat!(
    ...
    #[test]
    fn frontend_address_from_thrift_converts_host_and_port() {
        let thrift_addr = types::TNetworkAddress::new("fe.example".to_string(), 9030);

        let addr = frontend_address_from_thrift(&thrift_addr);

        assert_eq!(addr.hostname, "fe.example");
        assert_eq!(addr.port, 9030);
    }

    #[test]
    fn create_partition_result_from_wire_keeps_domain_metadata() {
        let response = frontend_service::TCreatePartitionResult {
            status: Some(status::TStatus::new(status_code::TStatusCode::OK, None)),
            partitions: Some(vec![descriptors::TOlapTablePartition::new(
                10,
                None,
                None,
                None,
                ...
                Some(vec![int_literal_node(100)]),
                Some(vec![int_literal_node(200)]),
                Some(vec![vec![string_literal_node("p1")]]),
                Some(true),
            )]),
            tablets: Some(vec![descriptors::TTabletLocation::new(30, vec![40, 41])]),
            nodes: Some(vec![descriptors::TNodeInfo::new(
                40,
                7,
        ...
        let result = create_partition_result_from_wire(response).expect("convert response");

        assert_eq!(result.partitions.len(), 1);
        assert_eq!(result.partitions[0].partition_id, 10);
        assert!(result.partitions[0].is_shadow);
        assert_eq!(result.partitions[0].indexes[0].index_id, 20);
        assert_eq!(result.partitions[0].indexes[0].tablet_ids, vec![30, 31]);
        assert_eq!(
            result.partitions[0].start_key,
            Some(vec![PartitionKeyValue::Int(100)])
        );
        assert_eq!(
            result.partitions[0].end_key,
            Some(vec![PartitionKeyValue::Int(200)])
        );
        assert_eq!(
            result.partitions[0].in_keys,
            vec![vec![PartitionKeyValue::Utf8("p1".to_string())]]
        );
        assert_eq!(result.tablets[0].tablet_id, 30);
        assert_eq!(result.tablets[0].node_ids, vec![40, 41]);
        assert_eq!(result.nodes[0].id, 40);
        assert_eq!(result.nodes[0].option, 7);
    }

    #[test]
    fn partition_key_conversion_handles_decimal_date_and_string_literals() {
        let decimal = decimal_literal_node("123.45", 12, 2);
        let date = date_literal_node("2026-06-30");
        let string = string_literal_node("us-east");

        let key = partition_boundary_key_from_wire(Some(&[decimal, date, string]), None)
            .expect("convert partition key")
            .expect("present key");
        ...
        assert_eq!(
            ...
            vec![
                ...
                PartitionKeyValue::Date32(20634),
                PartitionKeyValue::Utf8("us-east".to_string()),
            ]
        );
    }

    #[test]
    fn allocate_auto_increment_response_success_and_validation_errors() {
        let ok = frontend_service::TAllocateAutoIncrementIdResult {
            auto_increment_id: Some(42),
            allocated_rows: Some(3),
            status: Some(status::TStatus::new(status_code::TStatusCode::OK, None)),
        };
        assert_eq!(
            auto_increment_interval_from_wire(ok).expect("convert interval"),
            AutoIncrementInterval { next: 42, end: 45 }
        );

        let missing_status = frontend_service::TAllocateAutoIncrementIdResult {
            auto_increment_id: Some(1),
            allocated_rows: Some(1),
            status: None,
        };
        assert_eq!(
            auto_increment_interval_from_wire(missing_status).unwrap_err(),
            "alloc_auto_increment_id response missing status"
        );

        let invalid_rows = frontend_service::TAllocateAutoIncrementIdResult {
            auto_increment_id: Some(1),
            allocated_rows: Some(0),
            status: Some(status::TStatus::new(status_code::TStatusCode::OK, None)),
        };
        assert_eq!(
            auto_increment_interval_from_wire(invalid_rows).unwrap_err(),
            "alloc_auto_increment_id returned invalid allocated_rows=0"
        );
    }

    #[test]
    fn allocate_auto_increment_interval_retries_transport_error() {
        ...
        let server = FakeFeRpcServer::start(
            ...
            Box::new(move |method, seq, i_prot, o_prot| match method {
                "allocAutoIncrementId" => {
                    ...
                    assert_eq!(req.table_id, Some(11));
                    assert_eq!(req.rows, Some(4));
                    ...
                    let response = frontend_service::TAllocateAutoIncrementIdResult {
                        status: Some(status::TStatus::new(status_code::TStatusCode::OK, None)),
                        auto_increment_id: Some(100),
                        allocated_rows: Some(4),
                    };
        ...
        let addr = frontend_address_from_thrift(server.addr());

        let interval =
            allocate_auto_increment_interval(&addr, 11, 4).expect("retry auto increment RPC");
    ...
    fn int_literal_node(value: i64) -> exprs::TExprNode {
        exprs::TExprNode {
            ...
            type_: scalar_type_desc(types::TPrimitiveType::BIGINT),
            num_children: 0,
    ...
    fn decimal_literal_node(value: &str, precision: i32, scale: i32) -> exprs::TExprNode {
        exprs::TExprNode {
            node_type: exprs::TExprNodeType::DECIMAL_LITERAL,
            type_: scalar_type_desc_with_precision(
    ...
    fn date_literal_node(value: &str) -> exprs::TExprNode {
        exprs::TExprNode {
    ...
    fn scalar_type_desc(primitive: types::TPrimitiveType) -> types::TTypeDesc {
        scalar_type_desc_with_precision(primitive, 0, 0)
    }

    fn scalar_type_desc_with_precision(
        ...
        types::TTypeDesc {
            types: Some(vec![types::TTypeNode {
                ...
                scalar_type: Some(types::TScalarType::new(
                    primitive,
                    None::<i32>,
                    Some(precision),
                    Some(scale),
