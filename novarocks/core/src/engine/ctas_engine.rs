...
mod tests {
    ...
    #[test]
    fn actual_source_prepare_is_inert_and_hadoop_rejects_before_target_side_effects() {
        let state = Arc::new(crate::engine::StandaloneState::default());
        let command = CtasEngine::classify_ctas(
            &state,
            "CREATE TABLE IF NOT EXISTS ice.sales.dst AS SELECT 1 AS x",
        )
        .expect("classify CTAS")
        .expect("CTAS command");
        let mut settings = SessionOptimizerSettings::default();
        settings.enable_common_subexpr_reuse = Some(false);
        let source = CtasEngine::prepare_ctas_source(
            &state,
            PrepareCtasSourceRequest {
                command: command.clone(),
                current_catalog: Some("source_catalog".to_string()),
                current_database: "analytics".to_string(),
                query_options: None,
                execution: fe_execution(settings.clone()),
            },
        )
        .expect("prepare CTAS source");

        assert_eq!(source.facts.target_catalog, "ice");
        assert_eq!(source.facts.target_namespace, "sales");
        assert_eq!(source.facts.target_table, "dst");
        assert_eq!(
            source.facts.source_catalog.as_deref(),
            Some("source_catalog")
        );
        assert_eq!(source.facts.source_database, "analytics");
        assert_eq!(
            source.facts.execution_identity,
            source.handle.execution_identity()
        );
        assert_eq!(source.facts.output_columns.len(), 1);
        let retained = source
            .handle
            .as_any()
            .downcast_ref::<CorePreparedCtasSource>()
            .expect("core prepared source");
        assert!(!retained.gate.executed.load(Ordering::Acquire));
        {
            ...
            let admitted = admitted.as_ref().expect("inert admitted execution");
            assert_eq!(admitted.topology().targets().len(), 1);
            assert_eq!(admitted.optimizer_settings(), &settings);
        }
        ...
        assert_eq!(
            CtasEngine::precheck_ctas_target(&state, &command, None, "unused")
                .expect("typed target precheck"),
            CtasTargetPrecheck::Absent
        );
        let prepare = CtasEngine::prepare_ctas_target(
            &state,
            source.handle.as_ref(),
            ConnectorStagedCreateOperationId::new(),
            CreatePolicy::NoOpIfExists,
        );
        let Err(failure) = prepare else {
            panic!("Hadoop must reject staged CTAS target preparation")
        };
        assert_eq!(failure.kind, CtasFailureKind::Unsupported);
        assert!(!retained.gate.executed.load(Ordering::Acquire));
        assert_eq!(
            CtasEngine::precheck_ctas_target(&state, &command, None, "unused")
                .expect("target remains absent"),
            CtasTargetPrecheck::Absent
        );

        let second = CtasEngine::prepare_ctas_target(
            &state,
            source.handle.as_ref(),
            ConnectorStagedCreateOperationId::new(),
            CreatePolicy::NoOpIfExists,
        );
        let Err(failure) = second else {
            panic!("target prepare reservation must be exact-once")
        };
        assert_eq!(failure.kind, CtasFailureKind::InvalidRequest);
    }
