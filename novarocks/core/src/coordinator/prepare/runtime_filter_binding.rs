...
mod tests {
    ...
    #[test]
    fn prepared_table_type_cannot_express_dynamic_deployment_fields() {
        let source = include_str!("runtime_filter_binding.rs");
        let table = source
            .split("pub(crate) struct RuntimeFilterBindingTable")
            .nth(1)
            .unwrap()
            .split("impl RuntimeFilterBindingTable")
            .next()
            .unwrap();
        for forbidden in [
            "endpoint",
            ...
            assert!(
                ...
                "RuntimeFilterBindingTable must not express dynamic field {forbidden}"
            );
        }
    }

    #[test]
    fn preparation_has_one_binding_table_materialization_owner() {
        let owner = include_str!("runtime_filter_binding.rs")
            .split("#[cfg(test)]")
            .next()
            .unwrap();
        ...
        assert_eq!(
            entrypoint
                .matches("runtime_filter_binding::materialize_runtime_filter_binding_tables")
                .count(),
            1
        );
