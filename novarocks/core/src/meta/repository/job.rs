...
#[cfg(test)]
mod tests {
    use super::{JobState, StoredEraseJob, is_runnable};

    fn erase_job(state: JobState, retry_at_ms: Option<i64>) -> StoredEraseJob {
        StoredEraseJob {
            job_id: 1,
            table_id: 2,
            partition_id: None,
            root_path: "s3://bucket/table".to_string(),
            state,
            retry_at_ms,
            updated_at_ms: 0,
            last_error: None,
    ...
    #[test]
    fn erase_job_retry_is_runnable_only_when_due() {
        assert!(is_runnable(&erase_job(JobState::Pending, None), 100));
        assert!(!is_runnable(&erase_job(JobState::Failed, Some(101)), 100));
        assert!(is_runnable(&erase_job(JobState::Failed, Some(100)), 100));
        assert!(!is_runnable(&erase_job(JobState::Running, None), 100));
        assert!(!is_runnable(&erase_job(JobState::Finished, None), 100));
    }
}
