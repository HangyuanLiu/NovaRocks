...
pub fn eval_to_bitmap(
    ...
    if novarocks_types::largeint::is_largeint_data_type(input.data_type()) {
        let arr = novarocks_types::largeint::as_fixed_size_binary_array(&input, "to_bitmap")?;
        for row in 0..arr.len() {
            if arr.is_null(row) {
                builder.append_null();
                continue;
            }
            let raw = novarocks_types::largeint::value_at(arr, row)?;
