...
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub(crate) enum RuntimeDecimalWidth {
    Decimal32,
    Decimal64,
    Decimal128,
}
...
impl RuntimeFilterType {
    pub(crate) fn from_arrow_data_type(data_type: &DataType) -> Result<Self, String> {
    ...
    pub(crate) fn is_utf8_like(self) -> bool {
    ...
    pub(crate) fn decimal_width(self) -> Option<RuntimeDecimalWidth> {
        match self {
...
impl RuntimeDecimalWidth {
    pub(crate) fn for_precision(precision: u8) -> Result<Self, String> {
        match precision {
