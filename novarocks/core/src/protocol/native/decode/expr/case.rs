...
use super::{decode_expr, lower_required_unboxed_child};
...
pub(crate) fn lower_case(
    ...
    if let Some(operand) = &case_expr.operand {
        children.push(decode_expr(operand, arena, input_layout)?);
    }
    ...
    if let Some(else_expr) = &case_expr.else_expr {
        children.push(decode_expr(else_expr, arena, input_layout)?);
    }
    Ok(arena.push_typed(
        ExprNode::Case {
