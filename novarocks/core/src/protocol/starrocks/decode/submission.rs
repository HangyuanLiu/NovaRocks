...
use super::dependency::{
    FragmentExprArenaOwner, LakeMetaStorageFacts, QueryProfilePatch, StarRocksExternalDependency,
    ...
    for patch in &draft.query_profile_patches {
        ...
        query_profile_arena_mut(&mut draft.parts.program, patch.target().owner())?
            .replace_literal(
                ...
                crate::exec::expr::LiteralValue::Utf8(profile.clone()),
            )
            .map_err(|detail| {
...
fn query_profile_arena_mut(
    ...
    let arena = match (owner, sink) {
        (
            ...
            crate::exec::fragment::sink::FragmentSinkProgram::DataStream(program),
        ) => program.partition_arena_mut(),
        (
            FragmentExprArenaOwner::MultiCastDataStream,
            crate::exec::fragment::sink::FragmentSinkProgram::MultiCastDataStream(program),
        ) => program.partition_arena_mut(),
        (
            FragmentExprArenaOwner::SplitDataStream,
            crate::exec::fragment::sink::FragmentSinkProgram::SplitDataStream(program),
        ) => program.arena_mut(),
        (
            FragmentExprArenaOwner::IcebergTable,
            crate::exec::fragment::sink::FragmentSinkProgram::IcebergTable(program),
        ) => program.arena_mut(),
        (
            FragmentExprArenaOwner::IcebergChangeStreamRouter,
            crate::exec::fragment::sink::FragmentSinkProgram::IcebergChangeStreamRouter(program),
        ) => program.partition_arena_mut(),
        #[cfg(feature = "compat")]
        (
            FragmentExprArenaOwner::StarRocksOutputProjection,
            crate::exec::fragment::sink::FragmentSinkProgram::StarRocksTable(program),
        ) => {
...
mod tests {
    ...
    use crate::exec::fragment::program::{FragmentSinkKind, FragmentSinkSpec};
    ...
    #[cfg(feature = "compat")]
    use crate::connector::starrocks::sink::partition_key::PartitionExprPlan;
    #[cfg(feature = "compat")]
    ...
    fn assert_resolved_profile(arena: &ExprArena, expr_id: crate::exec::expr::ExprId) {
        assert!(matches!(
            arena.node(expr_id),
    ...
    #[cfg(feature = "compat")]
    fn starrocks_program_with_owned_arenas() -> (FragmentProgram, [crate::exec::expr::ExprId; 3]) {
    ...
    fn instance_first_decode_normalizes_hdfs_lake_and_schema_assignments() {
        ...
        for (node_id, kind) in [
            (18, crate::exec::fragment::program::ScanAssignmentKind::File),
            (
                19,
                crate::exec::fragment::program::ScanAssignmentKind::StarRocksTablet,
            ),
            (
                20,
                crate::exec::fragment::program::ScanAssignmentKind::SchemaSelection,
            ),
        ] {
    ...
    fn hdfs_candidate_node_survives_typed_assignment_into_runtime_cache_options() {
        ...
        let descriptors = descriptors::TDescriptorTable::new(
            vec![descriptors::TSlotDescriptor::new(
                1,
                1,
                bigint,
                0,
        ...
        let mut hdfs_scan = plan_nodes::THdfsScanNode::default();
        hdfs_scan.tuple_id = Some(1);
        hdfs_scan.hive_column_names = Some(vec!["k".to_string()]);
        let mut scan_node = empty_set_node();
        scan_node.node_id = 18;
        scan_node.node_type = plan_nodes::TPlanNodeType::HDFS_SCAN_NODE;
        scan_node.row_tuples = vec![1];
        scan_node.nullable_tuples = vec![false];
        scan_node.hdfs_scan_node = Some(hdfs_scan);
        let fragment = planner::TPlanFragment::new(
            plan_nodes::TPlan::new(vec![scan_node]),
            None,
            noop_sink(),
            partitions::TDataPartition::new(
                partitions::TPartitionType::UNPARTITIONED,
        ...
        let mut params = params(UniqueId { hi: 71, lo: 72 }, UniqueId { hi: 81, lo: 82 });
        ...
        let draft = prepare_fragment_submission(StarRocksDecodeInput {
            ...
            ..decode_input(&fragment, &params)
        })
        .expect("prepare HDFS fragment with candidate node");
        let decoded = finish_fragment_submission(draft, StarRocksResolvedDependencies::default())
            .expect("finish HDFS fragment with candidate node");
        let (submission, _) = decoded.into_parts();
        ...
        let Some(ScanMorsel::FileRange {
            ...
            ..
        }) = morsels.morsels.first()
        else {
            panic!("expected HDFS file morsel with range cache options");
        };
        assert_eq!(options.candidate_node.as_deref(), Some("backend-7"));
    }
    ...
    fn missing_plan_fails_before_missing_sink_with_exact_path() {
        ...
        assert_eq!(
            protocol.family(),
            crate::protocol::common::error::ProtocolFamily::StarRocks
        );
    }

    #[test]
    fn malformed_starrocks_fragment_has_zero_runtime_side_effects() {
        let query = QueryId { hi: 51, lo: 52 };
    ...
    fn external_dependencies_require_exact_resolution_before_submission() {
        let lake_request = super::super::dependency::LakeMetaStorageRequest::new(
            QueryId { hi: 71, lo: 72 },
            "catalog".to_string(),
            "db".to_string(),
            "table".to_string(),
