...
// to frontend session tests. They are never linked into the production wire
// server, which starts at `FrontendMysqlShim` above.
#[cfg(test)]
mod legacy {
    use super::*;
    ...
    fn apply_broadcast_profile_set(settings: &mut SessionOptimizerSettings, trimmed: &str) -> bool {
        ...
        if let Some(v) =
            parse_set_non_negative_integer(trimmed, "cbo_broadcast_node_mem_budget_bytes")
        {
    ...
    fn apply_optimizer_boolean_set(settings: &mut SessionOptimizerSettings, query: &str) -> bool {
        ...
        match name.as_str() {
            ...
            "enable_query_rewrite_table_prune" => {
                settings.enable_query_rewrite_table_prune = enabled
            }
    ...
    fn cancellation_error_for_mysql(reason: QueryCancellationReason) -> (ErrorKind, String) {
        match reason {
            ...
            QueryCancellationReason::DeadlineExceeded { timeout_ms } => {
                format_engine_error_for_mysql(EngineError::query_timeout(format!(
    ...
    async fn execute_statement_text(
        ...
        if let Some(v) =
            parse_set_non_negative_integer(trimmed, "global_runtime_filter_build_max_size")
        {
        ...
        if let Some(v) =
            parse_set_non_negative_integer(trimmed, "global_runtime_filter_build_min_size")
        {
        ...
        if let Some(v) =
            parse_set_non_negative_integer(trimmed, "global_runtime_filter_probe_min_size")
        {
        ...
        if let Some(v) =
            parse_set_non_negative_integer(trimmed, "cbo_max_reorder_node_use_exhaustive")
        {
    ...
    fn parse_admin_failpoint_query(query: &str) -> Result<Option<(String, FailPointMode)>, String> {
        ...
        let name = strip_string_quotes(parts[3]).ok_or_else(|| {
            "expected ADMIN ENABLE/DISABLE FAILPOINT '<failpoint_name>'".to_string()
        })?;
        if name.is_empty() {
    ...
    fn parse_admin_raise_engine_error_query(query: &str) -> Result<Option<EngineError>, String> {
        ...
        let err = match code {
            ...
            EngineErrorCode::IcebergWriteDescriptorMismatch => {
                EngineError::iceberg_write_descriptor_mismatch(
                    "forced P8 SQL runner error-code smoke",
                )
            }
    ...
    mod tests {
        ...
        fn request_query_options_projection_preserves_session_fields_and_runtime_defaults() {
            let actual = build_request_query_options(
                65_535,
                Some(60),
                Some(8),
                Some(250),
        ...
        fn parse_admin_raise_engine_error_rejects_unknown_code() {
            let err =
                parse_admin_raise_engine_error_query("ADMIN RAISE ENGINE ERROR 'NotARealCode'")
