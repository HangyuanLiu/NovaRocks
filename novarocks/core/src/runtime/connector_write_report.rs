...
pub(crate) struct ConnectorStagedReportCollector {
    inner: Arc<Mutex<Option<Vec<ConnectorStagedReportFrame>>>>,
}

impl ConnectorStagedReportCollector {
    pub(crate) fn record(&self, report: ConnectorStagedReport) -> Result<(), String> {
        report
            .validate()
    ...
    pub(crate) fn take(&self) -> Vec<ConnectorStagedReportFrame> {
        self.inner
            .lock()
