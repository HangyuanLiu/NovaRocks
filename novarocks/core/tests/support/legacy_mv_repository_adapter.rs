...
use novarocks::meta::repository::{RepositoryError, RepositoryErrorKind, RepositoryResult};
use novarocks::meta::{
    MetaError, MetaErrorKind, MetaReadTxn, MetaStoreProvider, MetaWriteTxn, SqliteMetaStoreProvider,
};
...
impl LegacyMvRepositoryAdapter {
    pub fn open(path: impl AsRef<Path>) -> Result<Self, MvRepositoryError> {
        Ok(Self {
            provider: SqliteMetaStoreProvider::open(path).map_err(map_meta_error)?,
            legacy: MvMetaRepository,
        })
    }
