...
#[test]
fn generic_avro_catalog_excludes_frontend_owned_mv_subjects() -> TestResult {
    let catalog = schema_catalog()?;
    for subject in [
        "mv.definition",
        "mv.target_lookup",
        ...
        assert!(
            catalog.latest(subject).is_err(),
            "generic metadata catalog must not own frontend MV subject `{subject}`"
        );
    }
    Ok(())
}
