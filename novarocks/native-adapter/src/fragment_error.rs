...
//! Native fragment wire-decoder errors.

use std::error::Error;
use std::fmt;

use novarocks_proto_codec::{FieldPath, FieldPathSegment, ProtocolError, ProtocolErrorKind};

/// Typed error used by nested native-protobuf leaf decoders before the owning
/// DTO boundary attaches its exact [`FieldPath`].
#[derive(Debug)]
pub struct NativeFragmentLeafDecodeError {
...
impl NativeFragmentLeafDecodeError {
    pub fn at_collection(kind: ProtocolErrorKind, detail: impl fmt::Display) -> Self {
        Self {
            kind,
    ...
    pub fn prepend_field(mut self, field: &'static str) -> Self {
    ...
    pub fn append_field(mut self, field: &'static str) -> Self {
        self.relative_path.push(FieldPathSegment::Field(field));
        self
    }

    pub fn append_index(mut self, index: usize) -> Self {
        self.relative_path.push(FieldPathSegment::Index(index));
        self
    }

    pub fn into_native(self, path: FieldPath) -> NativeFragmentDecodeError {
    ...
    /// Whether the leaf diagnostic contains a substring.
...
pub trait NativeFragmentErrorAtPath {
...
#[derive(Debug)]
pub enum NativeFragmentDecodeError {
...
impl NativeFragmentDecodeError {
    /// Whether the rendered error contains a diagnostic substring.
    pub fn contains(&self, pattern: &str) -> bool {
    ...
    pub fn missing(path: FieldPath, detail: impl fmt::Display) -> Self {
        Self::protocol_error(path, ProtocolErrorKind::MissingField, detail)
    }

    pub fn invalid_value(path: FieldPath, detail: impl fmt::Display) -> Self {
        Self::protocol_error(path, ProtocolErrorKind::InvalidValue, detail)
    }

    pub fn invalid_enum(path: FieldPath, detail: impl fmt::Display) -> Self {
        Self::protocol_error(path, ProtocolErrorKind::InvalidEnum, detail)
    }

    pub fn out_of_range(path: FieldPath, detail: impl fmt::Display) -> Self {
        Self::protocol_error(path, ProtocolErrorKind::OutOfRange, detail)
    }

    pub fn inconsistent(path: FieldPath, detail: impl fmt::Display) -> Self {
        Self::protocol_error(path, ProtocolErrorKind::InconsistentFields, detail)
    }

    pub fn unsupported(path: FieldPath, detail: impl fmt::Display) -> Self {
        Self::protocol_error(path, ProtocolErrorKind::Unsupported, detail)
    }

    pub fn map_invalid<T, E: NativeFragmentErrorAtPath>(
        path: FieldPath,
        result: Result<T, E>,
    ) -> Result<T, Self> {
