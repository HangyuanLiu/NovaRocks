...
/// Metadata table flavors for which the provider materializes manifest rows.
///
/// The planner-facing metadata enum remains in Core because it also describes
/// SQL-only table variants. This narrower provider contract carries only the
/// manifest walk formats that need external Iceberg I/O.
    ...
    for mf in manifest_list.entries() {
        ...
        for entry in manifest.entries() {
            ...
            match ty {
                ...
                MetadataTableType::Manifests => unreachable!("manifests handled above"),
            }
        }
    }
