...
pub const ICEBERG_POSITION_DELETE_FILE_PATH_FIELD_ID: i32 = 2_147_483_546;
pub const ICEBERG_POSITION_DELETE_POS_FIELD_ID: i32 = 2_147_483_545;
pub const ICEBERG_POSITION_DELETE_FILE_PATH_COLUMN: &str = "file_path";
pub const ICEBERG_POSITION_DELETE_POS_COLUMN: &str = "pos";

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct PositionDeleteDescriptorError {
    message: String,
}

impl PositionDeleteDescriptorError {
...
impl std::fmt::Display for PositionDeleteDescriptorError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(formatter, "{}: {}", self.code(), self.message)
    }
}

impl std::error::Error for PositionDeleteDescriptorError {}
...
fn descriptor_error(message: impl Into<String>) -> PositionDeleteDescriptorError {
    PositionDeleteDescriptorError {
...
) -> Result<(), PositionDeleteDescriptorError> {
...
pub fn output_schema_from_descriptor(
    desc: &PositionDeleteDescriptorInput,
) -> Result<SchemaRef, PositionDeleteDescriptorError> {
...
/// field names and IDs.
pub fn canonical_output_schema() -> SchemaRef {
...
) -> Result<PositionDeleteDescriptorBinding, PositionDeleteDescriptorError> {
    validate_required_fields(desc)?;
...
mod tests {
    ...
    fn descriptor_missing_file_path_field_id_fails() {
        ...
        assert_eq!(err.code(), "UnsupportedPositionDeleteDescriptor");
        assert!(err.to_string().contains("file_path field_id"));
    }
    ...
    fn descriptor_output_order_mismatch_fails() {
        ...
        let err = validate_required_fields(&desc).unwrap_err();
        assert_eq!(err.code(), "UnsupportedPositionDeleteDescriptor");
        assert!(err.to_string().contains("pos output_expr_index"));
    }
    ...
    fn descriptor_file_path_type_mismatch_fails() {
        ...
        let err = validate_required_fields(&desc).unwrap_err();
        assert_eq!(err.code(), "UnsupportedPositionDeleteDescriptor");
        assert!(err.to_string().contains("file_path type mismatch"));
    }
    ...
    fn descriptor_output_expr_count_mismatch_fails() {
        ...
        let err = bind_position_delete_descriptor(&desc, &expected).unwrap_err();
        assert_eq!(err.code(), "UnsupportedPositionDeleteDescriptor");
        assert!(err.to_string().contains("output expr count"));
    }
    ...
    fn descriptor_partition_source_field_id_mismatch_fails() {
        ...
        let err = bind_position_delete_descriptor(&desc, &expected).unwrap_err();
        assert_eq!(err.code(), "UnsupportedPositionDeleteDescriptor");
        assert!(err.to_string().contains("source field id"));
    }
