...
use novarocks::exec::expr::{ExprArena, ExprId, cast_with_special_rules};
...
fn read_file_payload(path: &str) -> Result<bytes::Bytes, String> {
    ...
    let provisional = access
        ...
        .map_err(|error| error.to_string())?;
    let size =
        novarocks::runtime::global_async_runtime::data_block_on(provisional.stat(&cancellation))?
            .map_err(|error| error.to_string())?;
    let file = access
        .bind(0, novarocks_fs::FileIdentity::new(path, size, None))
        .map_err(|error| error.to_string())?;
    novarocks::runtime::global_async_runtime::data_block_on(
...
impl ScanOp for FileLoadScanOp {
    fn execute_iter(
        &self,
        morsel: ScanMorsel,
        _profile: Option<novarocks::runtime::profile::RuntimeProfile>,
