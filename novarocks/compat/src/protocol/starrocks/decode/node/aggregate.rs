...
fn select_aggregate_inputs(
    ...
    let select_first_for_merge = |args: Vec<novarocks::exec::expr::ExprId>,
                                  name: &str|
     -> Result<Vec<novarocks::exec::expr::ExprId>, String> {
        let first = args
            .into_iter()
...
fn pack_struct_inputs(
    ...
    let struct_expr = arena.push_typed(
        novarocks::exec::expr::ExprNode::StructExpr { fields: args },
        struct_type,
    );
    Ok(vec![struct_expr])
}
...
mod tests {
    ...
    fn compat_aggregate_lowering_ignores_runtime_filters() {
        ...
        assert!(aggregate.runtime_filter_spec.is_empty());
    }
}
