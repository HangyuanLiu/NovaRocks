...
use novarocks::connector::iceberg::sink_plan::{
    ...
    IcebergSinkObjectStoreConfig, IcebergSinkPlan,
};
...
pub(crate) fn lower_iceberg_sink_factory_input(
    ...
    let lowered_partition_exprs = lower_partition_exprs(
        &partition_exprs,
        &mut arena,
        layout,
        last_query_id,
...
fn descriptor_error(message: impl Into<String>) -> novarocks::common::engine_error::EngineError {
    novarocks::common::engine_error::EngineError::unsupported_position_delete_descriptor(message)
}

fn required_output_index(
    label: &str,
    index: Option<i32>,
) -> Result<usize, novarocks::common::engine_error::EngineError> {
    let index =
...
) -> Result<DataType, novarocks::common::engine_error::EngineError> {
    type_desc
        .and_then(crate::protocol::starrocks::type_mapping::thrift_desc_to_arrow_type)
...
fn output_field_from_thrift(
    ...
    output_exprs: &[exprs::TExpr],
) -> Result<PositionDeleteOutputField, novarocks::common::engine_error::EngineError> {
...
fn position_delete_descriptor_input_from_thrift(
    ...
    output_exprs: &[exprs::TExpr],
) -> Result<PositionDeleteDescriptorInput, novarocks::common::engine_error::EngineError> {
    let desc =
    ...
    let partition_source_fields = desc
        ...
        .collect::<Result<Vec<_>, novarocks::common::engine_error::EngineError>>()?;
