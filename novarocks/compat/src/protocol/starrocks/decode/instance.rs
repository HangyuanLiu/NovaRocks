...
pub(crate) fn snapshot_decode_facts(
    ...
    let config = novarocks::common::app_config::config().map_err(|error| error.to_string())?;
    ...
    let datacache_available =
        config.runtime.cache.datacache_enable && datacache_block_cache_available();
        ...
        for slot in descriptors.slot_descriptors.as_deref().unwrap_or(&[]) {
            ...
            if !extended.contains(&raw_slot_id)
                ...
                || !super::layout::slot_name_from_desc(slot)
                    .is_some_and(|name| novarocks::exec::row_position::is_change_op(&name))
            {
                continue;
            }
...
pub(crate) fn decode_change_op_extended_column(
    ...
    novarocks::exec::change_op::validate_change_op_value(value)
