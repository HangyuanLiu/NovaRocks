...
impl ScanSourceFactory {
    ...
    pub(crate) fn with_operator_buffer_chunks(mut self, operator_buffer_chunks: usize) -> Self {
        self.operator_buffer_chunks = operator_buffer_chunks.max(1);
        self
    }
...
impl OperatorFactory for ScanSourceFactory {
    ...
    fn create(&self, _dop: i32, driver_id: i32) -> Box<dyn Operator> {
        ...
        Box::new(ScanSourceOperator {
            ...
            default_io_tasks: AtomicUsize::new(1),
            submit_fail_max: AtomicUsize::new(1),
            submit_fail_timeout_ms: AtomicU64::new(1),
...
struct ScanSourceOperator {
    ...
    default_io_tasks: AtomicUsize,
    submit_fail_max: AtomicUsize,
    submit_fail_timeout_ms: AtomicU64,
...
impl ScanSourceOperator {
    ...
    fn max_io_tasks_for_scan(&self) -> Result<usize, String> {
        ...
        let tasks = self
            ...
            .unwrap_or_else(|| self.default_io_tasks.load(Ordering::Acquire) as i32);
        if tasks <= 0 {
            return Err(format!(
    ...
    fn init_async_runners(&self, dispatch: Arc<ScanDispatchState>, max_io_tasks: usize) {
        ...
        for _ in 0..max_io_tasks {
            let runner = ScanAsyncRunner::new(
                ...
                self.driver_id,
                self.backend_num.load(Ordering::Acquire),
            );
            guard.push(runner);
        }
    }
    ...
    fn maybe_start_async_scan(&self) {
        ...
        let Some(scan_executor) = self.scan_executor.as_ref() else {
            self.async_state
                .set_error("scan execution requires an ExecutionRuntime".to_string());
            return;
        };
        ...
        loop {
            ...
            if !submitted {
                ...
                let fail_max = self.submit_fail_max.load(Ordering::Acquire).max(1);
                let fail_timeout_ms = self.submit_fail_timeout_ms.load(Ordering::Acquire).max(1);
...
impl Operator for ScanSourceOperator {
    ...
    fn bind_runtime_state(&mut self, state: &RuntimeState) -> Result<(), String> {
        self.backend_num
            .store(state.backend_num().unwrap_or(-1), Ordering::Release);
        if let Some(runtime) = state.execution_runtime() {
            self.scan_executor = Some(runtime.scan_executor());
            let config = runtime.config();
            self.default_io_tasks.store(
                usize::try_from(config.connector_io_tasks_per_scan_operator.max(1))
                    .expect("positive connector task count fits usize"),
                Ordering::Release,
            );
            self.submit_fail_max
                .store(config.scan_submit_fail_max.max(1), Ordering::Release);
            self.submit_fail_timeout_ms
...
mod tests {
    ...
    #[test]
    fn scan_produces_output_with_an_explicit_execution_runtime() {
        let rt = test_runtime_state();
        let op: Arc<dyn ScanOp> = Arc::new(TestMorselScanOp {
        ...
        assert!(
            output.is_some(),
            "scan should produce output through the explicitly supplied execution runtime"
        );
    }
