...
impl SpillStorageConfig {
    pub fn from_execution_runtime(
        runtime: &crate::runtime::execution_runtime::ExecutionRuntime,
    ) -> Result<Self, String> {
        let storage = &runtime.config().spill_storage;
        if !storage.enabled {
            return Err("spill storage is disabled by execution runtime".to_string());
        }
        let local_dirs = storage
            .local_dirs
            .iter()
            .cloned()
            .map(PathBuf::from)
        ...
        Ok(Self {
            local_dirs,
            dir_max_bytes: storage.dir_max_bytes,
...
impl Spiller {
    ...
    pub fn new_from_execution_runtime(
        runtime: &crate::runtime::execution_runtime::ExecutionRuntime,
        spill_encode_level: Option<i32>,
    ) -> Result<Self, String> {
        let storage = SpillStorageConfig::from_execution_runtime(runtime)?;
