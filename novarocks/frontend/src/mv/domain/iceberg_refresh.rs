...
impl MvEngine for StandaloneMvEngine {
    ...
    fn project_created_target(
        ...
        .map_err(|error| MvEngineError::new(MvEngineErrorKind::DescriptorSync, error.to_string()))
    }
...
fn base_fields_from_observation(
    fields: &[crate::mv::domain::storage_observation::MvObservedTargetField],
...
fn observed_base_fields<'a>(
    ...
    >,
    base_ref: &TableIdentity,
) -> Result<&'a [crate::mv::domain::storage_observation::MvObservedTargetField], String> {
...
fn drop_iceberg_mv_metadata_with_repository(
    ...
    let Some(loaded) = repository
        .find_by_target(&target)
        .map_err(|e| format!("load iceberg MV projection for drop failed: {e}"))?
    else {
        return Err(format!(
    ...
    let dropped = repository
        .delete_projection(
            uuid::Uuid::now_v7(),
            crate::mv::domain::repository::DeleteMvProjectionRequest {
