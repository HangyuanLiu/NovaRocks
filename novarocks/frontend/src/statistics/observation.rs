...
use novarocks::engine::statistics::{
    ...
    StatisticsLiteral, StatisticsOverwriteMode, StatisticsRequestContext,
...
use super::model::{
    AnalyzeStatusRow, ColumnStatRow, HistogramStatRow, MultiColumnStatRow, TableKey,
};
...
fn handle_analyze_statement(
    ...
    let target = StatisticsTableTarget {
        current_catalog: context.current_catalog.map(str::to_string),
        current_database: context.current_database.to_string(),
        name_parts: table_parts(&table)?,
    };
    let resolved_columns = engine.resolve_table_columns(&target)?;
    ...
    let columns = analyze_column_list(sql)?.unwrap_or_else(|| {
        resolved_columns
            .iter()
    ...
    let all_columns = columns.len() == resolved_columns.len();
    let status_columns = if all_columns {
        "ALL".to_string()
    } else {
        columns.join(",")
    };
...
fn drop_all_table_stats(service: &FrontendStatisticsService, key: &TableKey) {
    let mut state = service
        .state
        .write()
        .expect("frontend statistics write lock");
