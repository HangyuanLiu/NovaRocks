...
impl StateStoreHost {
    pub async fn open(
        ...
        config.validate()?;
        let provider_id = config.state_store.store.provider.provider_id();
        let cluster_id = config.state_store.store.cluster_id.clone();
        let bound = registry.bind(provider_id, &config)?;
        ...
        let validation = validate_open_instance(provider_id, instance.as_ref());
...
fn validate_open_instance(
    ...
    if instance.descriptor().id != provider_id {
