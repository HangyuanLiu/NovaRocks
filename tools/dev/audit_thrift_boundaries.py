...
BASELINE: dict[str, BaselineEntry] = {
    ...
    "src/connector/iceberg/schema.rs": BaselineEntry("domain-leak", "B1", 6, "Iceberg schema binding still reads TIcebergTable descriptors"),
    "src/connector/iceberg/position_delete_descriptor.rs": BaselineEntry("domain-leak", "B1", 13, "Position delete descriptor still validates thrift expr/type descriptors"),
    "src/connector/iceberg/position_delete.rs": BaselineEntry("domain-leak", "B1", 3, "Position delete path still touches thrift-derived descriptor data"),
    "src/connector/iceberg/equality_delete.rs": BaselineEntry("domain-leak", "B1", 2, "Equality delete path still touches thrift-derived descriptor data"),
    "src/connector/iceberg/changes.rs": BaselineEntry("domain-leak", "B1", 2, "Iceberg change path still shares thrift-facing delete/write descriptors"),
    "src/exec/row_position.rs": BaselineEntry("domain-leak", "B1", 1, "Row-position binding still participates in Iceberg delete descriptor conversion"),
    ...
    "src/connector/iceberg/state.rs": BaselineEntry("domain-leak", "B2", 2, "Iceberg state still carries thrift-facing write context"),
    "src/engine/iceberg_writer.rs": BaselineEntry("domain-leak", "B2", 14, "Standalone Iceberg writer still participates in thrift writer-report construction"),
    "src/engine/delete_flow.rs": BaselineEntry("domain-leak", "B2", 8, "Standalone delete flow still reaches thrift-facing Iceberg write descriptors"),
    ...
    "src/exec/runtime_filter/min_max.rs": BaselineEntry("domain-leak", "B3", 113, "Runtime min/max filter still stores thrift primitive type"),
    "src/exec/runtime_filter/bitset.rs": BaselineEntry("domain-leak", "B3", 32, "Runtime bitset filter still stores thrift primitive type"),
    "src/exec/runtime_filter/mod.rs": BaselineEntry("domain-leak", "B3", 16, "Runtime filter module still exports thrift primitive conversion"),
    "src/exec/runtime_filter/bloom.rs": BaselineEntry("domain-leak", "B3", 7, "Runtime bloom filter still stores thrift primitive type"),
    "src/exec/runtime_filter/membership.rs": BaselineEntry("domain-leak", "B3", 4, "Runtime membership filter still stores thrift primitive type"),
    "src/exec/runtime_filter/merger.rs": BaselineEntry("domain-leak", "B3", 3, "Runtime filter merger still sees thrift primitive type"),
    "src/runtime/runtime_filter_worker.rs": BaselineEntry("domain-leak", "B3", 1, "Runtime filter worker still touches thrift-facing filter data"),
    "src/exec/operators/hashjoin/hash_join_build_sink.rs": BaselineEntry("domain-leak", "B3", 5, "Hash join build sink still wires thrift-facing runtime filter data"),
    "src/exec/runtime_filter/codec.rs": BaselineEntry("legal-boundary", "B3-wire", 68, "Runtime filter codec is an explicit thrift/proto wire boundary"),
    ...
    "src/connector/starrocks/sink/factory.rs": BaselineEntry("domain-leak", "B4", 40, "StarRocks sink factory still accepts thrift sink and expr descriptors"),
    "src/connector/starrocks/sink/operator.rs": BaselineEntry("domain-leak", "B4", 3, "StarRocks sink operator still stores thrift where-clause state"),
    "src/connector/starrocks/sink/routing.rs": BaselineEntry("domain-leak", "B4", 13, "StarRocks sink routing still uses thrift network address payloads"),
    "src/connector/starrocks/sink/partition_key.rs": BaselineEntry("domain-leak", "B4", 12, "StarRocks partition key path still parses TExprNode literals"),
    "src/connector/starrocks/sink/auto_increment.rs": BaselineEntry("domain-leak", "B4", 5, "StarRocks auto-increment sink path still touches thrift sink descriptors"),
    "src/connector/hdfs.rs": BaselineEntry("domain-leak", "B5", 7, "HDFS scan planning still reads thrift descriptors directly"),
    "src/connector/iceberg/scan_planner.rs": BaselineEntry("domain-leak", "B5", 10, "Iceberg scan planner still reads thrift descriptors directly"),
    "src/runtime/lookup.rs": BaselineEntry("domain-leak", "B5", 8, "Runtime lookup still converts thrift descriptors into execution schema"),
    "src/exec/chunk/schema.rs": BaselineEntry("domain-leak", "B5", 7, "Execution chunk schema still contains thrift compatibility helpers"),
    "src/exec/operators/fetch_processor.rs": BaselineEntry("domain-leak", "B5", 4, "Fetch processor still consumes thrift-facing descriptor metadata"),
    "src/connector/scan_planning.rs": BaselineEntry("domain-leak", "B5", 2, "Shared scan planning still carries thrift descriptor metadata"),
    "src/connector/starrocks/table/scan_planner.rs": BaselineEntry("domain-leak", "B5", 2, "StarRocks table scan planning still carries thrift descriptor metadata"),
    "src/exec/node/scan.rs": BaselineEntry("domain-leak", "B5", 2, "Execution scan node still carries thrift-derived descriptor metadata"),
    "src/exec/operators/scan/runner.rs": BaselineEntry("domain-leak", "B5", 1, "Scan runner still carries thrift-derived descriptor metadata"),
    "src/runtime/query_context.rs": BaselineEntry("legal-boundary", "B5-wire", 10, "Query context may retain FE-compatible descriptor payload while normalized snapshots are introduced"),
    "src/connector/starrocks/lake/schema.rs": BaselineEntry("legal-boundary", "B6-wire", 87, "StarRocks lake schema is a storage/protocol adapter boundary"),
    "src/connector/starrocks/table/ddl.rs": BaselineEntry("legal-boundary", "B6-wire", 59, "StarRocks table DDL is a protocol adapter boundary"),
    ...
    "src/formats/starrocks/writer/segment_meta.rs": BaselineEntry("legal-boundary", "B6-wire", 23, "StarRocks segment writer emits storage-format type metadata"),
    "src/formats/parquet/mod.rs": BaselineEntry("legal-boundary", "B6-wire", 7, "Parquet format adapter still bridges thrift-derived type metadata"),
    ...
    "src/formats/parquet/variant_read.rs": BaselineEntry("domain-leak", "B6", 4, "Parquet variant reader still uses thrift primitive type as format logic"),
    ...
    "src/connector/starrocks/lake/context.rs": BaselineEntry("legal-boundary", "B6-wire", 2, "StarRocks lake context carries protocol adapter metadata"),
