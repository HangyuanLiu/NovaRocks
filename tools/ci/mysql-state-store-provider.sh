...
cargo test --test architecture_guard -- --list | \
  awk '$1 == "state_store_boundary::state_store_boundary_detector_rejects_foundationdb_owner_domain_and_forbidden_apis:" { n++ } END { exit(n != 1) }'
cargo test --test architecture_guard \
  state_store_boundary::state_store_boundary_detector_rejects_foundationdb_owner_domain_and_forbidden_apis -- --exact
