...
`94304f0154b6af25cc7d0378557ae2ab3ccd15ff` 依次 cherry-pick 与 candidate 相同的两个既有缺陷修复
（B0′ = `c3d39bf97995f2314298ba6f7117bb28f2d257b9`）；candidate 是开发完成后的任务分支 HEAD。负载、口径和门限
在看到 candidate 结果之前写定于 `workload.json`（`frozen_on`），测量启动后不修改；若必须修改，作废本轮并重新冻结。
下文与报告中的“B0”均指 B0′。

两个修复都由冒烟运行暴露，都与 UEA-4A-3 的改动无关，但不修则对照不成立：
...
- 结果一致性：每个查询记录结果行排序后的 sha256 前缀；每个负载在每一侧必须恰好一种结果，且两侧相同。
...
- 2026-09-25 初次冻结。
- 2026-09-25，任何正式测量之前：基线由 B0 改为 B0′（见开头），两次各加一个共享修复。
