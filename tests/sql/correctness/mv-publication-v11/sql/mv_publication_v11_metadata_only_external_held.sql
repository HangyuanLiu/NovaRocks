...
-- The external REST request freezes its original main condition at the
-- post-requirements service boundary. A filtered source change causes an
-- MV metadata-only publication with a new P and no changed target files;
-- that publication must defeat the frozen external request at JDBC CAS.
...
-- query 2
-- @skip_result_check=true
INSERT INTO fact VALUES ('west', -1);
...
-- query 4
-- @skip_result_check=true
-- @mv_rest_document_graph=ns_${uuid0}.target_mv,publications=2,metadata-only-last=true,failed-table-commits=1
