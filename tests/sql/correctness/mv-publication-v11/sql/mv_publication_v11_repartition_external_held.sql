...
-- and reaches the real service's post-requirements hold. The MV repartitions
-- and publishes P2 while that request is held. The original request must lose
-- JDBC CAS; the new partition spec and P2 must share one target commit.
...
-- query 4
-- @skip_result_check=true
-- @mv_rest_document_graph=ns_${uuid0}.target_mv,publications=2,interpretation-changes=true,failed-table-commits=1
...
SET CATALOG mvfrozen_${uuid0};
USE ns_${uuid0};
ALTER MATERIALIZED VIEW target_mv REPARTITION BY (truncate(k, 2));
...
if len(metadata["snapshots"]) != 2:
    sys.exit("repartition did not use one target snapshot")
spec_id = metadata["default-spec-id"]
spec = next((item for item in metadata["partition-specs"] if item["spec-id"] == spec_id), None)
if spec is None or not any(field["transform"] == "truncate[2]" for field in spec["fields"]):
    sys.exit(f"repartition did not install the requested partition spec: {spec!r}")
print("MV_FROZEN_EXTERNAL_REJECTED", expected, actual, spec_id)
' \
