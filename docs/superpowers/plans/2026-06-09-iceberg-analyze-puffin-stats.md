...
**Architecture:** 复用既有积木——`execute_query` 跑内部全表扫描、Theta sketch 计算、`StatsAssembler::write_puffin`、以及 `update_statistics().set_statistics().commit()` 提交模式。仅 iceberg 表走此路径；与 Spark `compute_table_stats` 模型一致（Puffin 只放 NDV，row_count/min-max 走 manifest）。设计见 `docs/superpowers/specs/2026-06-09-iceberg-analyze-puffin-stats-design.md`。
