...
**Goal:** 落地设计文档 `docs/superpowers/specs/2026-06-10-apply-correlated-subquery-framework-design.md` 的 M0 里程碑：`LogicalPlan::Apply` / `LogicalPlan::AssertOneRow` 两个新节点的全部类型系统接线、`SubqueryRewrite` 空 stage + `ApplyException` 兜底、`subquery_unnest_mode` session 变量。**零行为变化**——M0 结束时没有任何生产路径构造 Apply 或 AssertOneRow。
