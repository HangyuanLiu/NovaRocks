...
**Architecture:** vendored `iceberg` crate 的 `Catalog` trait 增加默认返回 FeatureUnsupported 的 view 方法;`iceberg-catalog-rest` 0.9.0 vendor 进仓库并实现 6 个 REST view endpoint(复用其 HttpClient 的认证/prefix);NovaRocks 侧依次为 registry 包装层(`views.rs`)、`CatalogBackend` trait、engine 路由(`statistics.rs` 前缀拦截 → `iceberg_view.rs`)与 SELECT 预分析展开(`iceberg_view_rewrite.rs`)。Spec: `docs/superpowers/specs/2026-06-10-iceberg-rest-view-design.md`。
