...
- **前缀集中会一次性触及多个 owner 模块**。这是纯机械改动，但它把「未登记 family 不可表达」从纪律变成结构，
  因此接受这次集中带来的 diff 面积。
- **登记表里有一条诚实的过渡项**：backend 的 durable 记录当前只含期望态（`backend_id`/`endpoint`/
  `desired_state`），因此被登记为 `ExternalProjection`。这不是它的目标形态——BE 自注册会整体重写这个 family。
  在那之前，「全局 `ProcessRuntime` 无持久 carrier」这句话**尚未整体成立**，本 ADR 不宣称它成立。
...
- 当 BE 自注册完成、backend family 被重写后：那条过渡登记项必须重新分类，「全局无持久运行态 carrier」才第一次
  可以被宣称。
