...
- vendored `iceberg::Transaction::commit` 只返回普通 `Result<Table>`，并按 retryable error 自动重试。
  它无法表达请求是否可能已经发出，因此不能直接作为 ADR-0104 所要求的 publication frontier。

ADR-0104 已把 lake publication 冻结为 `KnownUncommitted` / `CommitUnknown` / `KnownCommitted` 三态、
target exact OCC 与年龄窗 GC，并要求「不能证明所需路径的 provider 在零副作用点返回 typed `Unsupported`」。
ADR-0104 当时把不同 Catalog 的落地工作概括为「探测并缓存 capability matrix」——本 ADR 取代这句措辞，
...
再在 helper 里发现不支持）依然违反 ADR-0104 的零副作用 fail-fast 顺序。这是把问题改了个名字。

**C. 直接把 vendored `iceberg::Catalog` 和 `iceberg::Transaction` 暴露给各 operation family。** 代码最少。
但 vendored commit 的自动重试会在 dispatch certainty 未知时重发同一请求，直接违反 ADR-0104 中
...
exception、backend-specific stage-create fallback 与未闭合的 rollback cleanup 都不满足 ADR-0104 的强三态。
...
- provider-private `Transaction` 可以内部复用 vendored transaction/action/table commit，但**不得把裸
  ...
  ADR-0104 三态；`abort()` 只在 publication dispatch 前允许；`CommitUnknown` 之后 mutation authority
...
- **包装 transaction 是纯增量代码。** 它不带来任何新功能，唯一产出是 dispatch certainty 的可观察性。
  ...
  就是冗余的。现在写它，是因为 ADR-0104 的三态没有它就无法在 Iceberg 侧成立。
