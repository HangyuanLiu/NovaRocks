...
逻辑 catalog、namespace、table 名只回答“现在这个名字解析到哪里”，不能区分原表被 drop 后以同名 recreate 的 ABA。ADR-0075 已定义：durable caller 经既有 exact metadata lease 捕获并重绑 opaque `ConnectorTableObjectId`，以 typed `Missing` 或 `Replaced` 拒绝对象消失或替换；该 object identity 不等同于 schema version、data version 或逻辑表名。
...
**D：将 object capture/rebind 加入 statistics capability，或为 statistics 新建平行 resolver/lease。** 这会把 ADR-0075 的 metadata identity authority 重新附着到可选统计能力，复制装配与 conformance 责任，并让无统计 consumer 无法复用相同 identity contract。
...
- 经 ADR-0075 捕获的 opaque bounded physical `ConnectorTableObjectId`；
...
- statistics job 需要绑定历史 snapshot、branch、tag 或非 `Current` selector 时；必须扩展 ADR-0075 的 selector/proof 契约，而不能把它们伪装为 logical target。
...
- 多个非统计 durable consumer 需要相同的 attempt-time version resolution policy 时；可抽取 provider-neutral consumer guidance，但不得把 identity authority 从 ADR-0075 移入 statistics capability 或新增平行 lease。
