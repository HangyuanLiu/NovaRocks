...
一次逻辑查询与一次执行 attempt 不是同一个生命周期。逻辑查询拥有 SQL 语义、准确对象绑定、Connector 协商结论、输出契约和恢复政策；attempt 才拥有 `QueryExecutionId`、placement、Task identity、准确执行访问资格、split source、Runtime Filter deployment 与 transport owner。拓扑变化或 Worker 失败会替换 attempt，但不自动改变用户请求和已经证明的输入绑定。
...
| 生命周期 | 允许持有 | 不允许持有 |
|---|---|---|
| 查询观察与准备 | exact object binding、occurrence、统计和 MV 候选事实、协商 request/receipt、FE metadata principal | Worker placement、执行数据 secret、Task identity、运行 split source |
...
| attempt 实例化 | 本 attempt 的 topology、admission evidence、数据访问资格与作用域绑定、split source、Task/RF/Exchange manifest | 改写已冻结输出、残余谓词、对象版本或恢复承诺 |

FE 为 metadata、manifest 和统计读取使用进程自己的 metadata principal；每个 attempt 独立证明本次执行资格覆盖冻结绑定。静态 data credential 来自进程本地受控对象；vended 材料由实际签请求的 BE `StorageAuthority` 按需取得和续期。Catalog 可以提供独立 credentials endpoint，也可以只在 `load_table` 响应中返回 delegation；后者只使用 delegation，校验 table UUID 与授权资源覆盖冻结绑定，并忽略响应附带的 current snapshot。`metadata_location` 可以随正常提交变化，不能要求它等于冻结位置。authority 在 catalog generation、scope、能力路径与授权等价时可跨查询复用，不承诺每个 attempt 一次 Catalog HTTP。无法形成上述闭合证明时 fail closed。续期 owner 与有界调用见 [ADR-0151](ADR-0151-credential-renewal-is-driven-by-the-consumer.md)。
...
ADR-0129 曾通过“在 metadata materialization 前先 mint candidate attempt”解决 `load_table` 同时返回 metadata 与 vended credential 的问题。独立 FE metadata principal 和每次 attempt 的执行访问资格验证取消了这个前提：准备不再需要借用执行数据凭据。它关于秘密不进入 plan/cache、作用域精确、失败清理和不猜测 credential 的要求继续有效，并在本 ADR 中完整重述。
...
**选项二：把 vended credential、table runtime 或 split manager 保存在 plan/cache 中。** 它减少启动请求，却把短期材料放进语义缓存或可观察对象，且无法由 plan digest 安全表达。进程本地 `StorageAuthority` 的受控复用不需要把 secret 放入 plan/cache。由于违反秘密隔离和生命周期精确性，这是**设计否决**。
...
**选项四：冻结语义描述和 access recipe，每个 attempt 只实例化运行能力。** 准备有界交替到稳定后一次冻结；attempt 用准确 recipe 获取 topology、数据访问资格和 split source，并验证它们仍覆盖冻结事实。采纳。
...
4. **Per-attempt access rule。** 每个 attempt 根据冻结 recipe 独立建立数据访问资格与作用域绑定，并取得 split source、admission evidence、Task/RF/Exchange identity 与 transport owner；失败或取消由该 attempt owner 精确释放其运行责任。消费侧进程级 authority 可以在等价条件下复用，不要求每次向 Catalog 重取材料。
...
6. **Capability-matrix rule。** 静态 credential、credentials endpoint 和 delegation-only `load_table` 是封闭的支持形状；后者必须校验 object identity 和资源覆盖并忽略新 snapshot，不要求返回的当前 `metadata_location` 等于冻结位置。无法证明时 fail closed，不退化为重新规划或复用 metadata credential。
7. **Secret-isolation rule。** secret 不进入 frozen description、plan、digest、语义 cache、log、公开 handle 或 Native wire；执行材料留在实际签请求的消费进程受控 authority 中。planning error、cancel 与 attempt terminal 清理该 attempt 的运行引用；进程级 authority 按自身代际和生命周期收敛。
...
本 ADR 完整替代 ADR-0129。被替代的是“为了收集 vended credential，必须在 metadata materialization 前创建 attempt”的时序；继续保留并加强的是秘密隔离、准确授权范围、失败清理和无法证明时拒绝。执行材料留在消费进程，因而不再需要跨进程传输材料；Native 边界仍承担自身的鉴权与传输保护。

## 接受的妥协（诚实记录）

消费侧 authority 在材料不可用时可能发出 credentials 或 delegation 请求，attempt 也可能重新创建 split source。我们接受按需 Catalog 往返和首行延迟，以换取准备与 attempt 生命周期分离；不通过语义缓存 secret 或重做语义协商隐藏这笔成本。

delegation-only `load_table` 会返回一份当前 metadata，但消费侧取得只读取身份与 delegation。实现必须校验 UUID 与授权资源覆盖，且明确忽略新 snapshot；它不把当前 `metadata_location` 与冻结位置做相等检查。这比独立 credentials endpoint 更复杂，也受外部 Catalog response 形状约束。
...
3. 按需 credential 取得或每 attempt 的 split-source 建立在生产 profile 中成为可测瓶颈时，评估绑定 token、批量签发或受控 authority 复用；优化不能重新耦合 compiler 或把 secret 移入静态计划。
