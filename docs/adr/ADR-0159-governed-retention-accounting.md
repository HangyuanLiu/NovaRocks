...
4. **叶子 C/F/L 分工。** F 上的 CAS 裁决小额 grow 与返还/关闭竞争；L 用原子计数记录在途登记；C 只在慢路径中经账户树改变并以版本发布快照。慢补额先确保本请求的承诺并将本请求计入 L，才开放剩余 F。量化余量超水位时同步返还，父级仅取其自身可用 F，不侵占 floor、已外发容量或其他子级。关闭与最后一次释放用 CLOSED 上的一对原子读改写交接：释放先发布 F 再读改写 CLOSED，关闭先读改写 CLOSED 再扫 F，保证并发时至少一方接手返还；仅靠两个独立原子的 acquire/release 读写会漏返。
5. **分别观测与验收。** data、受治理 metadata、账户 C/L/F、CAS 重试、父链补额/返还、独立地址 census、物理分配与 holder 暴露分别报告。Linux 有间隔供给负载与端到端 filter/derive 成本按冻结的同线程数门槛验收；饱和紧循环只诊断。macOS 结果不能代替 Linux 裁决。
