---
id: ADR-0104
title: "Typed Connector Execution Binding Declaration"
...
code-anchors:
  - "novarocks/protocol/src/provider/execution_binding.rs (ConnectorExecutionBindingDeclaration)"
  - "novarocks/backend/src/connector/execution_host.rs (ConnectorExecutionHost)"
---

## 问题

Connector execution binding 如何在不把 credential、local client 或 provider private codec 带过 FE/BE 边界的前提下，形成唯一的 wire declaration、精确 generation 安装与可重放的失败语义？

## 背景与执行事实

原有 Ensure 请求把 provider 字符串、instance、incarnation 与 opaque bytes 分开传输；BE 再以字符串选择 installer。这样 Protocol 无法验证 provider-specific 结构，也无法对同一个 `(instance_id, incarnation)` 固定一个完整 declaration digest。Host 只有 Empty、Installing、Ready 三态，失败会回到 Empty，RPC 又把业务结果压成整数和文本。

ADR-0103 已把 provider wire authority 放在 Protocol，并要求同一个 native build island 内做原子 hard cut；ADR-0098 已确立 DTO 字段路径与验证错误由 Protocol 独占。本决策把这两个边界落实到 connector execution binding。

## 考虑过的选项

1. 保留 `provider_id + opaque payload`，只给外层加 digest。改动较小，但 Protocol 仍无法理解 provider declaration，SPI 与 provider codec 仍是并行 wire authority。
2. 在 SPI 建一个通用 Rust enum，再由 Protocol 复制转换。可给调用方强类型，但会复制 generated DTO、digest 与 validation，重新制造第二 authority。
3. 让 Protocol 持有 closed provider `oneof`、validated generated-root wrapper、canonical digest 和 typed outcomes；SPI 只 re-export 该值，Server 在启动时封闭 concrete installer set。该方案使 wire selector、结构校验和 error matrix 都能被独立验证。

## 裁决

采用选项 3。Ensure 只传 `execution_id` 与 typed declaration；Provider `oneof` 是唯一 dispatch selector。Protocol 对 canonical lowercase instance ID、16-byte incarnation、provider field bounds、reason/retry matrix 与 safe detail/path 做 fail-closed validation，并以 domain-separated canonical digest 覆盖完整 declaration root。
