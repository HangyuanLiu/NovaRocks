...
provenance:
  ...
  - "vault: 2026-07-21-runtime-filter-consolidated-umbrella-design (C5)"
...
1. **全部 BlockingSnapshot**（6G 之前的现状）：planner 无条件密封 blocking。在 CTE multicast 拓扑下与耦合反压组成真环（ADR-0002），被 6F 拒绝或在运行时吞成 timeout 延迟税。
...
3. **默认 Blocking + 定点 Live 降级**（选定）：结构必然的降级（纯 proof-backed 环内 consumer，等待可证明只能以超时收场）由 RFD-6G 在 planner seal 期静态执行；收益权衡型的选择（build 大/慢、RF 低选择性、深下推等「等不划算」的点位）留给后续 cost-based 决策（umbrella 工作项 C5）。

## 裁决

`BlockingSnapshot` 是默认——对绝大多数规划良好的 RF，「有界地等一下」拿到几乎全部剪枝红利；`NonBlockingLive{Batch}` 是定点手术刀，用在**等待不划算或不安全**的点位。降级决策静态密封进 plan（planner seal 期），不做运行时时序赌博。顺序：correctness/liveness-forced（6G，环内必须不等）先行，cost-based（C5，按收益选等或不等）后置。

## 接受的妥协（诚实记录）

- 在 C5 落地前，非环点位上「等不划算」的场景（build 大/慢、RF 低选择性）仍会白等 ≤timeout——这是把安全子集先做、收益子集后置的排期选择。
- Join 的 NonBlockingLive 执行路径（CompleteOnce live poll、batch 晚应用）是为定点降级新增的执行面，需要与 blocking 路径共用同一 predicate 编译以保证语义零漂移。
- 默认 blocking 意味着每个新的等待形态都要先通过 6F/6G 的静态 liveness 分析（见 ADR-0001 的前置税）。

## 何时重新评估

- C5 cost-based activation 落地时（默认策略从「一律 blocking」演进为「按收益密封」，本 ADR 的默认值部分将被 supersede）；
