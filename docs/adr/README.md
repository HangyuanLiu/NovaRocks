...
**怎么写**：用仓库内置的 `/adr` skill（`.agents/skills/adr/`，Codex 直接发现、Claude Code 经 `.claude/skills` → `.agents/skills` 软链发现）新建、supersede 或重编号——skill 内嵌模板与全部规则；人工手写请复制下方模板。正文中文；id/slug/tags/frontmatter 键用英文。
...
- ADR-0068 — Runtime Filter terminal observation 为何由 Backend participant 有界聚合、并仅经 typed QLC contribution 出域（active）
...
- ADR-0072 — SQL compiler 为何先完成全部 binding 分析物化、再冻结 statistics 并以无 catalog 的第二阶段优化封存（active）
...
- ADR-0025 — SQL compiler 为何以显式 request、immutable snapshots 与 post-compile binding context 形成唯一入口（superseded → ADR-0072）
