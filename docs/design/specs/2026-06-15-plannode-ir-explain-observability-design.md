# DistributedPlan IR + EXPLAIN Observability — Design
...
### 目标
- 引入真正的三层 IR：`PhysicalPlanNode → DistributedPlanNode/PlanFragment(单一来源) → thrift`，EXPLAIN 与执行都从 IR 派生。`node_id` 全程不重分配。
...
LAYER 1  PhysicalPlanNode (优化器输出, 不动)
           • Operator enum + Statistics + execution_props，TypedExpr + ColumnId
                 │  build_distributed_plan()  —— Pass 1（唯一结构遍历）
                 ▼
LAYER 2  DistributedPlanNode / PlanFragment / DistributedPlan  ←—— 单一来源 (src/sql/codegen/ir/**)
           • DistributedPlanNode: node_id, fragment_id, tuple_ids, nullable_tuple_ids, limit,
           ...
           • DistributedPlan: Vec<PlanFragment> + edges + root_fragment_id + rf_plan
                 │                                        │
        explain_distributed_plan(&fp)                   lower_distributed_plan(&fp)  —— Pass 2
                 ▼                                        ▼
...
optimize() → PhysicalPlanNode
               │ build_distributed_plan()   (Pass 1：结构/身份/拓扑)
               ▼
            DistributedPlan  ← 单一来源
            ┌──────────────┴───────────────┐
   explain_distributed_plan(&fp)          lower_distributed_plan(&fp)   (Pass 2：编译/绑定/序列化)
...
| `PlanNode`（抽象基类 + 每算子子类）| `DistributedPlanNode` struct + `DistributedPlanNodeBody` enum |
...
| `ExecPlan`（id 生成器 + DescriptorTable + fragments）| `DistributedPlan` + `DistributedPlanBuilder`/`LoweringCtx`（避开与运行时 `ExecPlan` 重名）|
| `PhysicalPlanTranslator.visit*` | `fragment_builder::DistributedPlanBuilder` 的 visit* |
| `PlanNode.treeToThrift()` | `lower_distributed_plan` / `DistributedPlanNode → TPlanNode`（Pass 2）|
| `getVerboseExplain()`/`getCostExplain()` | `DistributedPlanNode::explain(node, level, ctx)`|
...
pub(crate) struct DistributedPlanNode {
    ...
    pub children: Vec<DistributedPlanNode>,       // 仅 fragment 内子节点；exchange 是叶子，跨片连接在 DistributedPlan.edges
...
### 4.2 `DistributedPlanNodeBody`——enum 不用 trait
...
- Filter：**无 body 变体、不产 IR 节点**。Pass 1 把谓词折叠进子节点的 `DistributedPlanNode.conjuncts`（或 `ScanBody.predicates`），镜像今天 `visit_filter` 的 0-thrift-节点行为；谓词在子节点的 explain 行上呈现。这消除了"有 IR 节点却无 thrift 节点"的特例，使每个被 lower 的节点 1:1 对应 thrift。
...
### 4.4 `PlanFragment` / `DistributedPlan` / `DataSink` / `DataPartition`
...
`DistributedPlan` 等价 StarRocks `ExecPlan`；不复用 `ExecPlan` 名（NovaRocks 运行时已有 `ExecPlan{arena, root}`，`src/exec/node/mod.rs`）。

## 5. Build 流程：`fragment_builder` 变成 `PhysicalPlanNode → DistributedPlanNode` 翻译器

### 5.1 两段式（含 critique CRITICAL-1 的中心前提修正）

- **Pass 1 —— `build_distributed_plan`（结构遍历）**：单次走 `PhysicalPlanNode`，产 `DistributedPlan`。负责：**node_id / fragment_id / tuple_id 分配；分片（exchange/CTE 边界）；nullable-tuple 加宽指令；RF 拓扑；dict 流拓扑；连接器扫描结果（exec-param 种子）**。表达式以 `TypedExpr` + `ExprSource` 保留。**不分配 slot、不编译表达式。**
- **Pass 2 —— `lower_distributed_plan`（绑定/序列化）**：DFS 走 `DistributedPlan`，产 `MultiFragmentBuildResult`。负责：**slot 分配、descriptor table 构建、ExprCompiler `TypedExpr→TExpr` 编译、RF 描述符编译、dict 落地、`TPlanNode`/`TPlan`/`TDataSink`/`exec_params` 装配。**
...
| 关注点 | Pass 1（build_distributed_plan） | Pass 2（lower_distributed_plan） |
|---|---|---|
...
```rust
// Pass 1
pub(crate) struct DistributedPlanBuilder<'a> {
...
pub(crate) fn lower_distributed_plan(fp: &DistributedPlan, /*...*/) -> Result<MultiFragmentBuildResult, String>;
```

`PlanSubtree`（Pass-1 visit 返回）替代今天的 `VisitResult`，去掉 scope/slot 数据，携 IR `DistributedPlanNode` 树 + 输出 `ColumnId` + tuple_ids + CTE/ordering 簿记。

## 6. `to_thrift`（Pass 2）设计

`lower_distributed_plan(&DistributedPlan) -> MultiFragmentBuildResult` 再生今天的输出。逐 fragment、按 fragment 顺序：
...
### 7.1 VERBOSE 从 `DistributedPlan` 渲染

新 `explain_distributed_plan(fp, level) -> Vec<String>`，按 StarRocks 顺序走 fragment（root 最后构建、最先打印为 `PLAN FRAGMENT 0`）：
...
PLAN FRAGMENT <n>
  ...
  <root DistributedPlanNode 树, node_id 前缀>
```

per-node 渲染 `DistributedPlanNode::explain(node, level, ctx)`——一个 `match &node.body`，**复用 `format_physical_node` 现有富文本**（`explain.rs:455`），改读 `DistributedPlanNodeBody`。body 已按需携带今天 `format_physical_node` 读的字段（`HashJoinBody.exec_distribution` 喂 `join_distribution_label`、`ScanBody.min_max_hints` 喂 verbose min/max 行）。节点行加 **`node_id:` 前缀**（`<id>:HASH JOIN ...`），对齐 StarRocks `N:OP`，也与现有 logical `format_node` 的 `0:SCAN` 风格一致。`stats={rows=N}` trailer 从 `DistributedPlanNode.stats` 渲染。
...
- Normal 级也走 `explain_distributed_plan(fp, Normal)`，只是抑制 fragment 头/分布/stats trailer/RF 行，塌成单棵扁平树（带 node_id 前缀）。删除 `format_physical_node` 与 `explain_physical_plan`，消除双源。`explain_query`/`explain_analyze_query`（`engine/mod.rs:3289,3217`）改为 build IR 再调 `explain_distributed_plan`。
...
1. **Plan 一次**：`analyze → plan_query → optimize → build_distributed_plan` 产一个 `DistributedPlan`，explain 与执行都从它派生（消除三次 plan，含 :3280 的 boundary-schema 重建）。
2. **Lower 一次**：`lower_distributed_plan(&fp)`，经现有 `choose_standalone_execution` 执行。
...
6. **渲染 actual vs estimate**：`explain_distributed_plan(&fp, Analyze)` 走 IR，对每个节点用其 `node_id` 查 profile map，把 actual 贴在 estimate 旁：
   ```
   3:HASH JOIN (PARTITIONED, INNER, eq: [a=b])  est_rows=124  act_rows=131  time=2.3ms  peak_mem=4MB
   ```

**关联是 1:1**（因 Filter 已折叠进 conjuncts、不再有"无 thrift 节点的 IR 节点"，§4.3）：被 lower 的 fragment 内每个 DistributedPlanNode 的 `node_id` 都等于其 thrift `TPlanNode.node_id`、也等于 profile 的 `plan_node_id`，无翻译、无缺口。唯一例外是 **direct-exec fragment（AggregateStateMerge，IMV 路径）**：整片走 `DirectExecPlan`、不产 thrift 节点、不参与 per-node profile 关联，ANALYZE 对其作 fragment 级特殊渲染（不逐节点贴 actual）。query 级 `Planning/Execution/Rows` 头保留。
...
目标：`build_distributed_plan` + `lower_distributed_plan` 完全替代 eager thrift 发射，产规范化等价的 `MultiFragmentBuildResult`，套件全绿。**无用户可见变化。**
...
- **M1-S1** `explain_distributed_plan` 骨架 + 全级别 `node_id:` 前缀 + `PLAN FRAGMENT N` 头/分布/sink；per-node 文本从 `format_physical_node` 移植到 `DistributedPlanNode::explain`。
...
- thrift 由 IR 再生（完全忠实 `PhysicalPlanNode → DistributedPlanNode → thrift`），但 Pass 2 内部 compile-then-bind，等价性以规范化对拍判定。
...
- Pass 1：`src/sql/codegen/fragment_builder.rs`（`DistributedPlanBuilder`，替代 eager `VisitResult`/`visit` `:65`/`:1413`）
